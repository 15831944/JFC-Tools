...
//	short x=0;
	...
	CString TxtMois;
	TxtMois = m_LibMois[i][m_FormatMois];
	TxtMois.MakeUpper();
	if(size.cx+2<=l && size.cy<=h)
		//dc->DrawText(m_LibMois[i][m_FormatMois], &R, DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX);
		dc->DrawText(TxtMois, &R, DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX);
	dc->SetTextColor(oldColor);
	return(1);
}
...
//	ASSERT(AfxCheckMemory());
	...
	//COleDateTime DateTest(2001, 1, 1, 0, 0, 0);
	//int ValTest = atoi(DateTest.Format("%W"));

	//COleDateTime DateTest(2002, 1, 1, 0, 0, 0);
	//int ValTest = atoi(DateTest.Format("%W"));
	...
	for(i=0 ; i<m_nbcases ; i++)
	{
		...
		int ValSem = atoi(m_jourdeb.Format("%W"));

		COleDateTime DateTest(m_SCalend[i].Annee,1,1,0,0,0);
		int ValTest = atoi(DateTest.Format("%W"));

		// Modif Sept 2005 (si Lundi ou Mardi ou Mercredi ou Jeudi 1er jour de l'année, >> Semaine commence en 1)
		//                 (si Vendredi ou Samedi ou Dimanche >> Semaine commence en 0 ou 53) 
		int JourWeek = DateTest.GetDayOfWeek();

		if (ValTest == 0)
		{
			if(JourWeek >= 2 && JourWeek <= 5)
				m_SCalend[i].NoSem++;
		}

		// Avant Modif 20 Sept 2004
		//if (ValTest == 0)
		//{
		//	m_SCalend[i].NoSem++;
		//}
