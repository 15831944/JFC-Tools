	...
	public class DataBaseMBS
	{
		...
		public class Support
		{
			...
			public void AddTarif(DateTime date, uint formatIdentifier, uint color, double tarif)
			{
				...
				// on verifie si une version de tarif existe à cette date d'application
				if ((found = m_Tarifs.Values.FirstOrDefault(v => v.m_Date == date)) == null)
				{
					uint codeTarif;
					Dictionary<KeyValuePair<uint, uint>, Tarif> duplicatedTarifs = new Dictionary<KeyValuePair<uint, uint>, Tarif>();

					if (m_Tarifs.Count > 0)
					{
						// on se base sur la derniere version de tarif existante
						uint last = m_Tarifs.Max(v1 => v1.Key);

						// on duplique les tarifs de la version precedante
						duplicatedTarifs = m_Tarifs[last].m_tarifs
							.ToDictionary(kvp => kvp.Key,
							kvp => new Tarif()
							{
								m_FormatIdentifier = kvp.Value.m_FormatIdentifier,
								m_ColorCode = kvp.Value.m_ColorCode,
								m_Tarif = kvp.Value.m_Tarif
							});

						// on incremente la version de tarif
						codeTarif = last + 1;
					}
					else
					{
						// on commence à la version 1
						codeTarif = 1;
					}
				...
				else
				{
					...
					found.m_tarifs[key].m_Tarif = tarif;
				}
			}
		}
		...
		public Dictionary<string, string> m_ParutionsMappings;
		...
		public DataBaseMBS(DateTime date, DataSupports supports, DataVersions versions, DataTarifs tarifs, DataParutions parutions)
		{
			m_Date = date;
			m_Data = new Dictionary<string,Support>();
			m_SupportMappings = new Dictionary<string, string>();
			m_ParutionsMappings = new Dictionary<string, string>();
		...
		private void Import(DataSupports supports, DataVersions versions, DataTarifs tarifs, DataParutions parutions)
		{
			...
			m_ParutionsMappings = new Dictionary<string, string>(parutions.Mappings);
			...
			m_supportsIgnored = new List<string>(supports.m_SupportsIgnored);
		}
		...
		public void write(string directory)
		{
			...
			DataSupports.write(directory, m_Date, supports, m_SupportMappings, m_supportsIgnored);
			...
			DataParutions.write(directory, m_Date, parutions, m_ParutionsMappings);
		}
		...
		public void AddSupportMapping(string supportToMap, string mappedSupport)
		{
			if (!m_SupportMappings.ContainsKey(supportToMap))
				...
				m_SupportMappings[supportToMap] = mappedSupport;
		}
		...
		public void AddColorMapping(string couleurToMap, string mappedCouleur)
		{
			if (!m_colorMappings.ContainsKey(couleurToMap))
				...
				m_colorMappings[couleurToMap] = mappedCouleur;
		}
		...
		public void AddFormatMapping(string formatToMap, string mappedFormat)
		{
			if (!m_formatsMappings.ContainsKey(formatToMap))
				...
				m_formatsMappings[formatToMap] = mappedFormat;
		...
		internal void updateParutions()
		{
			...
			m_Data.Values.ToList().ForEach(support =>
			{
				...
				if (m_ParutionsMappings.ContainsKey(support.m_Libelle))
				{
					var match = m_ParutionsMappings[support.m_Libelle];
					found = mappings.Single(m => m["supports codipresse"] == match);
				}

				if (found != null && support.m_Tarifs.Count > 0)
				{
					Parution from;
					int to;//annee max
