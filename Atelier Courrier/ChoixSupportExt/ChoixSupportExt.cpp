// ChoixSupportExt.cpp : définit les fonctions d'initialisation pour la DLL.
//
...
static AFX_EXTENSION_MODULE ChoixSupportExtDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de ChoixSupportExt.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(ChoixSupportExtDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ChoixSupportExtDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Arrêt de ChoixSupportExt.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(ChoixSupportExtDLL);
	}
	return 1;   // ok
}
