// JFCTablesPresse.cpp : définit les fonctions d'initialisation pour la DLL.
//
...
static AFX_EXTENSION_MODULE JFCTablesPresseDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de JFCTablesPresse.DLL !\n");
		
		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(JFCTablesPresseDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(JFCTablesPresseDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Arrêt de JFCTablesPresse.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(JFCTablesPresseDLL);
	}
	return 1;   // ok
}
