// RepSupportsExt.cpp : définit les fonctions d'initialisation pour la DLL.
//
...
static AFX_EXTENSION_MODULE RepSupportsExtDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RepSupportsExt.DLL !\n");
		
		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RepSupportsExtDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RepSupportsExtDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Arrêt de RepSupportsExt.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RepSupportsExtDLL);
	}
	return 1;   // ok
}
