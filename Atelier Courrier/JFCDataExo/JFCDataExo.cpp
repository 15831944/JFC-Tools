// JFCDataExo.cpp : définit les fonctions d'initialisation pour la DLL.
//
...
static AFX_EXTENSION_MODULE JFCDataExoDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de JFCDataExo.DLL !\n");
		
		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(JFCDataExoDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(JFCDataExoDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Arrêt de JFCDataExo.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(JFCDataExoDLL);
	}
	return 1;   // ok
}
