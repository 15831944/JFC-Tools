// SQNAdVisionDlg.cpp : fichier d'implémentation
//
...
// boîte de dialogue CSQNAdVisionDlg

const CRect CSQNAdVisionDlg::s_rectDefault(95, 80, 286, 195);  // statique
//			+191 +115
//0095 0080 0286 0195

CSQNAdVisionDlg::CSQNAdVisionDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CSQNAdVisionDlg::IDD, pParent)
{
	m_RepItems  = 0;
	m_Advision  = 0;
...
void CSQNAdVisionDlg::LoadApps()
{
	m_RepItems  = new CRepItems();
	m_Advision  = new CAdvision();// new CAnalyseur();
}


CSQNAdVisionDlg::~CSQNAdVisionDlg()
{
	delete m_Advision;
	delete m_RepItems;
}

void CSQNAdVisionDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CSQNAdVisionDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
...
// gestionnaires de messages pour CSQNAdVisionDlg


void CSQNAdVisionDlg::SetCmdInfo(CATPComm *cmdInfo)
{
	m_pcmdInfo = cmdInfo;
}


BOOL CSQNAdVisionDlg::OnInitDialog()
{
	CDialog::OnInitDialog();
...
void CSQNAdVisionDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CSQNAdVisionDlg::OnAboutbox()
{
	CAboutDlg dlgAbout;
	dlgAbout.DoModal();
}
...
//  la fenêtre réduite.
HCURSOR CSQNAdVisionDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CSQNAdVisionDlg::SetupControls()
{
...
void CSQNAdVisionDlg::SetColors()
{
...
void CSQNAdVisionDlg::OnBnClickedAtpRepitem()
{
...
void CSQNAdVisionDlg::OnBnClickedAtpAnalyseur()
{
	if (!m_Advision->GetState())
		m_Advision->Show();
	else
		m_Advision->Hide();
}

BOOL CSQNAdVisionDlg::ContinueModal()
{
	if (bFisrt)
	{

		// Lancement Répertoire Cibles
		if (m_pcmdInfo->m_appToStart == CATPComm::COM3_ITEM)
		{
			m_RepItems->Show();
		}

		// Lancement des Tris Croises
		if (m_pcmdInfo->m_appToStart == CATPComm::COM3_ANALYSEUR)
		{
			m_Advision->Show();
			if(m_pcmdInfo->m_strFileName != "")
				m_Advision->Open(m_pcmdInfo->m_strFileName);
		}
...
void CSQNAdVisionDlg::OnAtpPath()
{
	CPathDlg pathdlg;
...
LRESULT CSQNAdVisionDlg::OnMessageFromModule(WPARAM wParam, LPARAM lParam)
{
	m_RepItems->ForwardMessage(GetCurrentMessage()->message, wParam, lParam);
	m_Advision->ForwardMessage(GetCurrentMessage()->message, wParam, lParam);
	return 0;
}


void CSQNAdVisionDlg::OnDestroy()
{
    CString strText;
	...
	AfxGetApp()->WriteProfileString("Options\\SQNAdVision", "Rect", strText);
    //AfxGetApp()->WriteProfileInt("Options\\ATP", "Icon", bIconic);
    AfxGetApp()->WriteProfileInt("Options\\SQNAdVision", "Max", bMaximized);

	CDialog::OnDestroy();
}

void CSQNAdVisionDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
	CString strText;
    BOOL bIconic, bMaximized;
    UINT flags;
    ...
    if (m_bFirstTime)
	{
        ...
        bIconic = AfxGetApp()->GetProfileInt("Options\\SQNAdVision", "Icon", 0);
        bMaximized = AfxGetApp()->GetProfileInt("Options\\SQNAdVision", "Max", 0);   
        if (bIconic)
		{
...
void CSQNAdVisionDlg::OnSysColorChange()
{
	CDialog::OnSysColorChange();
	SetColors();
}

void CSQNAdVisionDlg::OnAtpSeuilstat()
{
	CDlgSeuilsStats dlg;
...
void CSQNAdVisionDlg::OnClose()
{
	// on ferme l'analyseur
	if (m_Advision->CanClose()) m_Advision->Close(); else return;
...
BOOL CSQNAdVisionDlg::PreTranslateMessage(MSG* pMsg)
{
