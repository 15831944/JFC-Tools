// JFCSupports.cpp : définit les fonctions d'initialisation pour la DLL.
//
...
static AFX_EXTENSION_MODULE JFCSupportsDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de JFCSupports.DLL !\n");
		
		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(JFCSupportsDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(JFCSupportsDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Arrêt de JFCSupports.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(JFCSupportsDLL);
	}
	return 1;   // ok
}
