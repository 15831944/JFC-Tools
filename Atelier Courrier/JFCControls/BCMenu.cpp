...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		size_t size = sizeof(wchar_t)*(wcslen(szWideString)+1);
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		nLen = 0;
		size_t size = wcslen((wchar_t *)pTp)+1;
		szCaption=new wchar_t[size];
		wcscpy_s(szCaption, size, (wchar_t *)pTp);
		pTp=&pTp[size*sizeof(wchar_t)];//modified SK
	...
	for(int i=0;i<(int)GetMenuItemCount();++i){
		...
		if(GetSubMenu(i)){
			...
			ModifyMenu(i,MF_POPUP|MF_BYPOSITION,
				(UINT)(UINT_PTR)GetSubMenu(i)->m_hMenu,str);
		}
		else{
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			if(NULL==pSubMenu){
				...
				for(i=(int)m_MenuList.GetUpperBound();i>=0;i--){
					if(m_MenuList[i]->nID==(UINT)(UINT_PTR)pSubMenu->m_hMenu){
						delete m_MenuList.GetAt(i);
						m_MenuList.RemoveAt(i);
						break;
					}
...
BOOL BCMenu::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	...
	if(MF_BYPOSITION&nFlags)
	{
		UINT numMenuItems = (unsigned int)(int)m_MenuList.GetUpperBound();
		if(id<=numMenuItems){
#ifdef UNICODE
...
BOOL BCMenu::TrackPopupMenu( UINT nFlags, int x, int y, CWnd* pWnd, LPCRECT lpRect )
{
    CullDisabledItems(this, pWnd);
    return CMenu::TrackPopupMenu( nFlags,x,y,pWnd,lpRect);
}


void BCMenu::CullDisabledItems(CMenu* pMenu, CWnd* pWnd)
{
    CCmdUI state;
    UINT n;
    ...
    for(n =0; n < pMenu->GetMenuItemCount(); n++)
    {
        ...
        else
        {
			...
			//pMenu->GetMenuState(MF_BYPOSITION, UINT nFlags );
			UINT stateMenu = pMenu->GetMenuState(n, MF_BYPOSITION);
			ASSERT(stateMenu != 0xFFFFFFFF);
			if (stateMenu & MF_DISABLED)
			//if( !state.m_bEnabled )
            {
