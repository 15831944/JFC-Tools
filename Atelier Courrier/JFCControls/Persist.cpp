...
const CRect CPersistentFrame::s_rectDefault(0,  0,
                                            640, 480);  // statique
...
void CPersistentFrame::OnDestroy()
{
    ...
    WRITEPROFILESTRING(s_profileHeading, s_profileRect, strText);
    //Je ne veux pas sauver le statut iconic
	//WRITEPROFILEINT(s_profileHeading, s_profileIcon, bIconic);
    WRITEPROFILEINT(s_profileHeading, s_profileMax, bMaximized);
...
void CPersistentFrame::ActivateFrame(int nCmdShow)
{
    ...
    if (m_bFirstTime) {
        m_bFirstTime = FALSE;
        strText = GETPROFILESTRINGD(s_profileHeading, s_profileRect, 0);
        if (!strText.IsEmpty()) {
		...
		//Je ne veux pas sauver le statut iconic
		//bIconic = GETPROFILEINTD(s_profileHeading, s_profileIcon, 0);
		bIconic = FALSE;
        bMaximized = GETPROFILEINTD(s_profileHeading, s_profileMax, 0);   
        if (bIconic) {
