...
void JFCListBoxS::SetCount(LONG nombre, BOOL update)
{
	...
	if (update) this->InvalidateRect(NULL, FALSE);
}

LONG JFCListBoxS::GetCount()
{
	// on vérifie la validité de la fenêtre
	ASSERT(::IsWindow(this->m_hWnd));
...
BEGIN_MESSAGE_MAP(JFCListBoxS, JFCViewportSelect)
	//{{AFX_MSG_MAP(JFCListBoxS)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
const TCHAR* JFCListBoxS::OnGetItemLibelle(LONG indice)
{
...
LONG JFCListBoxS::OnGetItemSelection(LONG indice)
{
...
bool JFCListBoxS::OnBeginSelecting(LONG & etat)
{
	// on arrête la sélection des éléments
	return (false);
}

void JFCListBoxS::OnSelectItem(LONG indice, LONG etat)
{
	// on ne fait rien
}

void JFCListBoxS::OnEndSelecting()
{
	// on ne fait rien
}
...
bool JFCListBoxS::OnBeginDrawing(CDC & dc)
{
...
void JFCListBoxS::OnDrawState(CDC & dc, RECT rect, LONG i, LONG j, LONG etat)
{
...
void JFCListBoxS::OnDrawItem(CDC & dc, RECT rect, LONG i, LONG j)
{
	RECT cadre;
...
void JFCListBoxS::OnEndDrawing(CDC & dc)
{
...
LONG JFCListBoxS::OnGetItemSelection(LONG i, LONG j)
{
	// on renvoie l'état de l'élément
	return (this->OnGetItemSelection(j));
}

void JFCListBoxS::OnSelectItem(LONG i, LONG j, LONG etat)
{
...
/////////////////////////////////////////////////////////////////////////////
// JFCListBoxS message handlers

void JFCListBoxS::PreSubclassWindow() 
{
	// on initialise le nombre d'éléments
	m_Count =  0;

	// on appelle le gestionnaire de base
	this->JFCViewportSelect::PreSubclassWindow();
