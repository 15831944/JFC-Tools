...
void ETSLayoutMgr::EraseBkgnd(CDC* pDC)
{
	...
	HBRUSH hBrBack = (HBRUSH) (DWORD_PTR)::GetClassLong(GetWnd()->GetSafeHwnd(), GCL_HBRBACKGROUND) ;
	if( hBrBack == 0 )
...
int ETSLayoutMgr::Pane::getMaxConstrainHorz() 
{
	if(m_Orientation == HORIZONTAL) {
		...
		return (nMaxConstr == -1) ? -1 : (int)(nMaxConstr + (m_paneItems.GetUpperBound()*m_sizeBorder) + (int)(2*m_sizeExtraBorder));
	}
