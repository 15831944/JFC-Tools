...
bool CResizer::Add(const CBorderInfo *pBorders, int nSize)
{
  if (m_vInfo)
  {
	TInfo vInfoOld = m_vInfo;
	int SizeOld = m_nSize;
	 
	m_vInfo = new CControlInfo[nSize+SizeOld];
	m_nSize = nSize+SizeOld;
	  
	for(int i=0; i<SizeOld; i++)
	{
		m_vInfo[i] = vInfoOld[i];
		CControlInfo &ci = m_vInfo[i];
		GetDlgItemRect(ci.pInfo->nID, ci.rcInitial);
	#ifdef _DEBUG
		//Make some debug checking
	...
	{
		CControlInfo &ci = m_vInfo[i+SizeOld];
		ci.pInfo = pBorders + i;
		GetDlgItemRect(ci.pInfo->nID, ci.rcInitial);
	#ifdef _DEBUG
		//Make some debug checking
...
void CResizer::HideCtrl() const
{
	for (int index = 0; index < m_nSize; index++)
	{
		const CControlInfo &ci = m_vInfo[index];
		CRectInfo &ri = m_vRectInfo[index];
		ri.nID = ci.pInfo->nID;
...
void CResizer::MoveAndHideOverlapped() const{
  ...
  // ::ShowWindow(m_wndParent,SW_SHOW); 
}
