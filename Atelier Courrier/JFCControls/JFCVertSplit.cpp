...
JFCVertSplit::JFCVertSplit(UINT Message) : m_Msg(Message)
{
	// on initialise les éléments
	m_Track = FALSE; m_TrackPos = m_DeltaPos = m_DispRight = m_DispLeft = 0;
}
...
BOOL JFCVertSplit::SetRange(LONG Right, LONG Left)
{
	...
	m_DispRight = Right; m_DispLeft = Left;
...
JFCVertSplit::~JFCVertSplit()
{
	// on ne fait rien
}

BEGIN_MESSAGE_MAP(JFCVertSplit, JFControl)
	//{{AFX_MSG_MAP(JFCVertSplit)
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
...
LONG JFCVertSplit::OnDrawRectangle(LONG Mode)
{
	RECT  Rect;
	POINT Point;
	...
	if (Larg > 0 && Haut > 0)
	{
		...
		if (Mode == 1)
		{
			...
			m_TrackPos = Rect.left; m_DeltaPos = Point.x - Rect.left;

			// on effectue le premier tracé
			Dc.Rectangle(m_TrackPos, Rect.top - 1, m_TrackPos + Larg, Rect.bottom + 1);

			// on modifie le déplacement
			Disp = m_TrackPos - Rect.left;
		}
		else if (Mode == 2)
		{
			// on calcule la nouvelle position
			LONG NewPos = Point.x - m_DeltaPos;

			// on corrige la position pour la conserver des les limites
			if      (NewPos > Rect.left + m_DispRight) NewPos = Rect.left + m_DispRight;
			else if (NewPos < Rect.left - m_DispLeft ) NewPos = Rect.left - m_DispLeft;
			...
			if (NewPos != m_TrackPos)
			{
				...
				Dc.Rectangle(NewPos, Rect.top - 1, NewPos + Larg, Rect.bottom + 1);
...
/////////////////////////////////////////////////////////////////////////////
// JFCVertSplit message handlers

void JFCVertSplit::PreSubclassWindow() 
{
	// on initialise les éléments
	m_Track = FALSE; m_TrackPos = m_DeltaPos = m_DispRight = m_DispLeft = 0;
...
void JFCVertSplit::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// on teste si on peut effectuer un déplacement
	if (m_DispRight > 0 || m_DispLeft > 0)
	{
...
void JFCVertSplit::OnLButtonUp(UINT nFlags, CPoint point) 
{
	// on teste si on peut effectuer un déplacement
	if (m_DispRight > 0 || m_DispLeft > 0)
	{
...
void JFCVertSplit::OnMouseMove(UINT nFlags, CPoint point) 
{
	// on teste si on peut effectuer un déplacement
	if (m_DispRight > 0 || m_DispLeft > 0)
	{
