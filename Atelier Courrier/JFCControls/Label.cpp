...
// 1.1
	...
	// Mode pour le remplissage
	m_CATPColorsApp = CATPColors::COLORNOAPP;
	// m_CATPColorsApp = CATPColors::COLORLANCEUR;
	// m_ColorDegrad	= FALSE;
	m_ModeDegrad    = CATPColors::LIGHT_DEGRAD;
}
...
void CLabel::OnPaint() 
{
	...
	if (!m_bTransparent)
	{
		if (m_fillmode == Normal)
		{
			...
			{
				// Calcul le tableau des couleurs dégradées
				// static COLORREF clr_degrad[CATPColors::m_NbGrad];
				// static COLORREF clr_degrad[15];

				// CATPColors::GetColorLightGraduation(CATPColors::COLORANALYSEUR, clr_degrad, CATPColors::m_NbGrad);

				/*
				if (m_ModeDegrad == CATPColors::LIGHT_DEGRAD)
				{
					CATPColors::GetColorLightGraduation(m_CATPColorsApp, clr_degrad, CATPColors::m_NbGrad);
				}
				else if (m_ModeDegrad == CATPColors::MEDIUM_DEGRAD)
				{
				...
				// Coloris le rectangle avec la graduation couleur
				CATPColors::FillRectGraduate(*pDCMem, rc, clr_degrad, CATPColors::m_NbGrad, CATPColors::FROM_TOP_DEGRAD);
				*/

				CATPColors::FillRectModeGraduate(*pDCMem, rc, m_CATPColorsApp, m_ModeDegrad);

				// Ajout Liseret autour
				CPen pen3Border		(PS_SOLID, 0, CATPColors::GetColorDark(m_CATPColorsApp));	// Border color

				// on dessine les bords haut et gauche en foncé
				pDCMem->SelectObject(&pen3Border);
				pDCMem->MoveTo(rc.left , rc.bottom-1);
				pDCMem->LineTo(rc.left , rc.top);
				pDCMem->LineTo(rc.right-1 , rc.top);

				// on dessine le bord bas en plus foncé
				CPen pen3DarkBorder		(PS_SOLID, 0, CATPColors::GetColorSuperDark(m_CATPColorsApp));	// Dark Border color
				pDCMem->SelectObject(&pen3DarkBorder);
				pDCMem->MoveTo(rc.right-1 , rc.top);
				pDCMem->LineTo(rc.right-1	, rc.bottom-1);
				pDCMem->LineTo(rc.left	, rc.bottom-1);
			}
		}
	...
	if (style & SS_CENTERIMAGE)
	{
	 ...
	 //ajout d'un bord au CLabel pour plus de relief (activé en appelant SetBorderColor)
	 //cela n'a strictement rien à voir avec SetBorder

	if (m_crBorderSet == TRUE) 
	{
		...
		CPen* pOldPen = pDCMem->SelectObject(&pen3HighLight);
		pDCMem->MoveTo(rc.left +1	, rc.top + 1);
		pDCMem->LineTo(rc.right - 1	, rc.top + 1);
		pDCMem->LineTo(rc.right - 1	, rc.bottom - 1);					
		pDCMem->LineTo(rc.left +1	, rc.bottom - 1);					
		pDCMem->LineTo(rc.left +1	,  rc.top + 1);			
	...
	else
	{
		...
		pDCMem->DrawText(strText,nRC,dwFlags);
		if (m_bFont3d)
		{
	...
	for(int x=0; x < m_hWindows.GetCount(); x++)
	{
		if(m_hWindows[x])
		{
			if(IsWindow(m_hWindows[x]))
			{
				CWnd *pWnd = FromHandle(m_hWindows[x]);
				if(pWnd)
					pWnd->RedrawWindow(0,0,RDW_ERASE|RDW_FRAME|RDW_INVALIDATE|RDW_UPDATENOW);
			}
			else
				TRACE("Handle not window %d\n",x); 
		}
	}
}

void CLabel::AddTopWindow(HWND hWnd)
{
	m_hWindows.Add(hWnd);
}
...
//////////////////////////////////////////////////////////////////////////
//
// Function:		CLabel::SetBorderColor
//
...
// RETURNS:         Reference to 'this' object
//
// NOTES:			If you don't call that methods on a CLabel, the label won't
//					appear with the borders, and will be totally flat-like.
// 
// MODIFICATIONS:
//
// Name                     Date        Version Comments
// A RAINONE				21/04/08    1.0     Origin
//
//////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
/*
CLabel& CLabel::SetBkColor2(COLORREF crBkgnd, 
							CATPColors::APPCOLOR AppColor, 
							CATPColors::MODE_DEGRAD ModeDegrad, 
							BOOL Degrad,
						    COLORREF crBkgndHigh, BackFillMode mode)
*/
						   ...
						   COLORREF crBkgndHigh, BackFillMode mode)
{
	...
	if (AppColor == 0)
		...
		m_CATPColorsApp = CATPColors::COLORREPSUPPORTS;
	else if (AppColor == 3)
		...
		m_CATPColorsApp = CATPColors::COLORZOOM;
	else if (AppColor == 7)
		m_CATPColorsApp = CATPColors::COLORNOAPP;
						   ...
						   COLORREF crBkgndHigh, BackFillMode mode)
{
	...
	m_CATPColorsApp = CATPColors::COLORLANCEUR;
	m_ModeDegrad	= CATPColors::LIGHT_DEGRAD;
	// m_ColorDegrad   = FALSE;   // pas de graduation couleur
