...
IMPLEMENT_DYNAMIC(JFCSplash, CWnd)
JFCSplash::JFCSplash()
{
	m_pWndParent=NULL;
	m_strText="";
	...
	m_nyPos=0;
	m_dwTimeout=2000;
	m_FadeInTime=m_FadeOutTime=500;
	m_dwStyle=0;
...
BOOL JFCSplash::Create(CWnd *pWndParent,LPCTSTR szText,DWORD dwTimeout,DWORD dwStyle)
{
...
BOOL JFCSplash::SetBitmap(UINT nBitmapID,short red,short green,short blue)
{
	BITMAP bm;
...
BOOL JFCSplash::SetBitmap(LPCTSTR szFileName,short red,short green,short blue)
{
	BITMAP bm;
	HBITMAP hBmp;
...
void JFCSplash::SetTextFont(LPCTSTR szFont,int nSize,int nStyle)
{
	LOGFONT lf;
...
void JFCSplash::SetTextDefaultFont()
{
	LOGFONT lf;
...
void JFCSplash::SetText(LPCTSTR szText)
{
	m_strText=szText;
	RedrawWindow();
}

void JFCSplash::SetTextColor(COLORREF crTextColor)
{
	m_crTextColor=crTextColor;
	RedrawWindow();
}

void JFCSplash::SetTextRect(CRect& rcText)
{
	m_rcText=rcText;
	RedrawWindow();
}

void JFCSplash::SetTextFormat(UINT uTextFormat)
{
	m_uTextFormat=uTextFormat;
}

void JFCSplash::SetFadeTime(DWORD ms)
{
	m_FadeInTime = m_FadeOutTime = ms;
}

void JFCSplash::SetFadeInTime(DWORD ms)
{
	m_FadeInTime = ms;
}
void JFCSplash::SetFadeOutTime(DWORD ms)
{
	m_FadeOutTime = ms;
}
...
HRGN JFCSplash::CreateRgnFromBitmap(HBITMAP hBmp, COLORREF color)
{
...
void JFCSplash::DrawWindow(CDC *pDC)
{
	CDC memDC;
	CBitmap *pOldBitmap;
...
BEGIN_MESSAGE_MAP(JFCSplash, CWnd)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
...
// JFCSplash message handlers

BOOL JFCSplash::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}

void JFCSplash::OnPaint()
{
	CPaintDC dc(this); // device context for painting
	DrawWindow(&dc);
}

LRESULT JFCSplash::OnPrintClient(WPARAM wParam, LPARAM lParam)
{
...
BOOL JFCSplash::PreTranslateMessage(MSG* pMsg)
{
...
void JFCSplash::OnTimer(UINT nIDEvent)
{
	KillTimer(0);
	Hide();
}

void JFCSplash::PostNcDestroy()
{
	CWnd::PostNcDestroy();
	delete this;
}