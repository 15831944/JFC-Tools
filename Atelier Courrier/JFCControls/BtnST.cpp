...
// CButtonST::CButtonST(CATPColors::APPCOLOR AppColor)
CButtonST::CButtonST()
{
	// Par défaut aucune application
	m_CATPColorsApp		= CATPColors::COLORNOAPP;

	m_bIsPressed		= FALSE;
	m_bIsFocused		= FALSE;
	m_bIsDisabled		= FALSE;
	m_bMouseOnButton	= FALSE;
	m_dwTextAlign		= DT_CENTER;// Text centered by default
	...
	// By default the button is not an onglet, et non actif
	m_bIsOnglet			= FALSE;
	m_bIsOngletActif	= FALSE;

	// By default bouton ordinaire sans lien menu (pas de coin en bas à droite)
	m_bIsBtnMenu		= FALSE;

	// By defaut pas de bouton flèche, ni à gauhe, ni à droite
	m_bIsFlecheG		= FALSE;		
	m_bIsFlecheD		= FALSE;		

	// By default pas de progress bar
	m_bIsModeProgress	= FALSE;
	m_PrctProgress		= 0;
...
BEGIN_MESSAGE_MAP(CButtonST, CBtnSTLabel)
...
void CButtonST::PreSubclassWindow() 
{
	...
	if (m_nTypeStyle == BS_CHECKBOX )
		m_bIsCheckBox = TRUE;
	if (m_nTypeStyle == BS_AUTOCHECKBOX )
		m_bIsCheckBox = TRUE;
	if (m_nTypeStyle == BS_RADIOBUTTON )
		m_bIsCheckBox = TRUE;
	if (m_nTypeStyle == BS_AUTORADIOBUTTON )
		m_bIsCheckBox = TRUE;
	...
	CBtnSTLabel::PreSubclassWindow();
} // End of PreSubclassWindow

UINT CButtonST::OnGetDlgCode() 
{
	UINT nCode = CBtnSTLabel::OnGetDlgCode();
...
BOOL CButtonST::PreTranslateMessage(MSG* pMsg) 
{
	...
	return CBtnSTLabel::PreTranslateMessage(pMsg);
...
{
	UINT nNewType = (wParam & static_cast<UINT>(BS_TYPEMASK));
...
void CButtonST::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	...
	if (!bSetFlag) CBtnSTLabel::OnMeasureItem(nIDCtl, lpMeasureItemStruct);
} // End of OnMeasureItem
#endif

void CButtonST::OnEnable(BOOL bEnable) 
{
	CBtnSTLabel::OnEnable(bEnable);
...
void CButtonST::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	CBtnSTLabel::OnMouseMove(nFlags, point);
...
{	
	...
	//else
	{
...
#endif
		{
			...
			DWORD dwRetValue = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON /*| TPM_NONOTIFY */| TPM_RETURNCMD, rWnd.left, rWnd.bottom, pWndMenuParent, NULL);
#else
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | m_dwTextAlign | DT_CALCRECT);
	if(m_dwTextAlign == DT_CENTER)
		rpCaption->OffsetRect((centerRect.Width() - rpCaption->Width())/2, (centerRect.Height() - rpCaption->Height())/2);
	else
		rpCaption->OffsetRect(5, (centerRect.Height() - rpCaption->Height())/2);
	/* RFU
	...
	*/
	if (m_bIsDisabled)
	{
		...
		pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | m_dwTextAlign);
		rpCaption->OffsetRect(-1, -1);
...
/////////////////////////////////////////////////////////////////
// Bouton de type onglet ???
DWORD CButtonST::SetTypeOnglet(BOOL bOnglet, BOOL bOngletActif, BOOL bRepaint)
{
	// Est ce un bouton type onglet, et est-il actif ???
	m_bIsOnglet		 = bOnglet;
	m_bIsOngletActif = bOngletActif;
...
DWORD CButtonST::SetTypeFleche(BOOL AGauche, BOOL ADroite, BOOL bRepaint)
{
	// Fleche à gauche <== (texte calé à droite)
	m_bIsFlecheG	= AGauche;
	if (AGauche)
	{
		SetTextAlignement(DT_CENTER);
		// alignement à droite ne marche pas
		// SetTextAlignement(DT_RIGHT);
	}

	// Fleche à droite ==> (texte calé à gauche)
	m_bIsFlecheD	= ADroite;
	if (ADroite)
		SetTextAlignement(DT_LEFT);
...
/////////////////////////////////////////////////////////////////
// A FAIRE
// Bouton de type entete menu ??? (dessin avec encoche)
DWORD CButtonST::SetTypeBtnMenu(BOOL bMenu,BOOL bRepaint)
{
	m_bIsBtnMenu = bMenu;
...
//////////////////////////////////////////////////////////////////
// Bouton de type progress
DWORD CButtonST::SetTypeBtnProgress(BOOL bProgress, BOOL bRepaint)
{
	m_bIsModeProgress = bProgress;
...
DWORD CButtonST::SetDefaultColors(BOOL bRepaint)
{
	...
	m_crColors[BTNST_COLOR_FG_FOCUS]	= ::GetSysColor(COLOR_BTNHIGHLIGHT);
	m_crColors[BTNST_COLOR_SEL]			= ::GetSysColor(COLOR_BTNFACE);
	m_crColors[BTNST_COLOR_TEXT]		= ::GetSysColor(COLOR_BTNTEXT);
...
//			Invalid color index.
//
DWORD CButtonST::SetColor(BYTE byColorIndex, COLORREF crColor, CATPColors::APPCOLOR AppColor, BOOL bRepaint)
{
	// Attributs pour traiter le mode dégradé
	m_CATPColorsApp = AppColor;
	
	// A VIR ERm_ModeDegrad	= Degrad;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CButtonST::SetMenu(UINT nMenu, CWnd* pParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
	...
	if (nMenu)
	{
		...
		m_hParentWndMenu = pParentWnd->GetSafeHwnd();
	} // if
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	CBitmap bm;
	CATPColors::MODE_DEGRAD ModeDegrad = CATPColors::LIGHT_DEGRAD;

	if (m_bIsPressed)
	{
		crColor = m_crColors[BTNST_COLOR_BK_IN];
		bm.LoadBitmap(IDB_BLEU_LIGHT_DEGRADE);
		ModeDegrad = CATPColors::LIGHT_DEGRAD;
	}
	else
	{
		if (m_bMouseOnButton)
		{
			...
			ModeDegrad = CATPColors::DARK_DEGRAD;
		}
		else
		{
			crColor = m_crColors[BTNST_COLOR_BK_OUT];
			bm.LoadBitmap(IDB_BLEU_MEDIUM_DEGRADE);
			ModeDegrad = CATPColors::MEDIUM_DEGRAD;
		}
	} // else

	// Couleur progression calcul
	crProgress = m_crColors[BTNST_COLOR_BK_OUT];
	...
	/*
	CBrush brBackground;
	brBackground.CreatePatternBrush(&bm);
	*/

	CBrush      brProgress(crProgress);

	if (m_bIsOnglet == FALSE)
	{
		if (m_bIsFlecheG == FALSE && m_bIsFlecheD == FALSE)
		{
			if (m_CATPColorsApp == CATPColors::COLORNOAPP)
			{
				// Bouton Rectangulaire Ordinaire
				pDC->FillRect(pRect, &brBackground);
			}
			else
			{
				// Applique dégradé de couleur
				CATPColors::FillRectModeGraduate(*pDC, *pRect, m_CATPColorsApp, ModeDegrad);
			}
			
			// Si bouton avec menu déroulant
			if (m_bIsBtnMenu == TRUE)
			{
				// Dessin coin signal menu
				CPoint ptPolygone[3];
				ptPolygone[0] = CPoint(pRect->right - 2, pRect->bottom - 2);
				ptPolygone[1] = CPoint(pRect->right - pRect->Height()/3, pRect->bottom - 2);
				ptPolygone[2] = CPoint(pRect->right - 2, pRect->bottom - pRect->Height()/3);

				CRgn RgnPolygone;
				RgnPolygone.CreatePolygonRgn(ptPolygone, 3 ,WINDING);

				/*
				CBrush brBtnShadow(GetSysColor(COLOR_3DDKSHADOW));
				pDC->FillRgn(&RgnPolygone, &brBtnShadow);
				*/

				crColor = m_crColors[BTNST_COLOR_SEL];
				CBrush brBtnShadow(crColor);
				pDC->FillRgn(&RgnPolygone, &brBtnShadow);
			}
			else if (m_bIsModeProgress)
			{
				// Affiche une progression
				CPoint ptPolygone[4];
				ptPolygone[0] = CPoint(pRect->left, pRect->top);
				ptPolygone[1] = CPoint(pRect->right * m_PrctProgress / 100, pRect->top);
				ptPolygone[2] = CPoint(pRect->right * m_PrctProgress / 100, pRect->bottom);
				ptPolygone[3] = CPoint(pRect->left, pRect->bottom);

				CRgn RgnPolygone;
				RgnPolygone.CreatePolygonRgn(ptPolygone, 4 ,WINDING);

				// Si progression non fini, on passe en couleur clair
				if (m_PrctProgress < 100.0)
					pDC->FillRgn(&RgnPolygone,&brProgress);
				else
					pDC->FillRgn(&RgnPolygone,&brBackground);

			}
		}
		else if (m_bIsFlecheG)
		{
			// Bouton Fleche Gauche - Remplissage du fond
			CPoint ptPolygone[5];
			ptPolygone[0] = CPoint(pRect->right, pRect->top);
			// ptPolygone[1] = CPoint(pRect->left + (pRect->right - pRect->left) / 4, pRect->top);
			ptPolygone[1] = CPoint(pRect->left + 20, pRect->top);
			ptPolygone[2] = CPoint(pRect->left, pRect->top + (pRect->bottom - pRect->top) / 2);
			// ptPolygone[3] = CPoint(pRect->left + (pRect->right - pRect->left) / 4, pRect->bottom);
			ptPolygone[3] = CPoint(pRect->left + 20, pRect->bottom);
			ptPolygone[4] = CPoint(pRect->right, pRect->bottom);
			CRgn RgnPolygone;
			RgnPolygone.CreatePolygonRgn(ptPolygone, 5 ,WINDING);
			...
			else
			{
				...
				{
					CATPColors::GetColorLightGraduation(m_CATPColorsApp, clr_degrad, CATPColors::m_NbGrad);
				}
				else if (ModeDegrad == CATPColors::MEDIUM_DEGRAD)
				{
					CATPColors::GetColorMediumGraduation(m_CATPColorsApp, clr_degrad, CATPColors::m_NbGrad);
				}
				else
				{
					CATPColors::GetColorDarkGraduation(m_CATPColorsApp, clr_degrad, CATPColors::m_NbGrad);
				}
				CATPColors::FillRgnPolygoneGraduate(*pDC, RgnPolygone, clr_degrad, CATPColors::m_NbGrad, CATPColors::STYLE_DEGRAD::FROM_TOP_DEGRAD);
			}
			*/
		}
		else if (m_bIsFlecheD)
		{
			// Bouton Fleche Droite - Remplissage du fond
			CPoint ptPolygone[5];
			ptPolygone[0] = CPoint(pRect->left, pRect->top);
			// ptPolygone[1] = CPoint(pRect->right - (pRect->right - pRect->left) / 4, pRect->top);
			ptPolygone[1] = CPoint(pRect->right - 20, pRect->top);
			ptPolygone[2] = CPoint(pRect->right, pRect->top + (pRect->bottom - pRect->top) / 2);
			// ptPolygone[3] = CPoint(pRect->right - (pRect->right - pRect->left) / 4, pRect->bottom);
			ptPolygone[3] = CPoint(pRect->right - 20, pRect->bottom);
			ptPolygone[4] = CPoint(pRect->left, pRect->bottom);
	...
	{
		// Bouton Onglet, remplissage du fond 
		CPoint ptPolygone[4];
		ptPolygone[0] = CPoint(pRect->left, pRect->top);
		ptPolygone[1] = CPoint(pRect->right, pRect->top);
		ptPolygone[2] = CPoint(pRect->right - 6, pRect->bottom);
		ptPolygone[3] = CPoint(pRect->left + 6, pRect->bottom);
		...
		/*
		if (m_bIsOngletActif == FALSE)
		{
			// Onglet non actif, on dessine tout le contour
			CBrush brBtnShadow(GetSysColor(COLOR_3DDKSHADOW));
			pDC->FrameRgn(&RgnPolygone, &brBtnShadow, 1, 1);
		}
		else
		{
			// Onglet actif, on ne dessine pas le bord supérieur 
			CPen pen3DDKShadow(PS_SOLID, 0, GetSysColor(COLOR_3DDKSHADOW)); // Black
			CPen* pOldPen = pDC->SelectObject(&pen3DDKShadow);
			pDC->MoveTo(pRect->left, pRect->top);
			pDC->LineTo(pRect->left + 6, pRect->bottom-1);
			pDC->LineTo(pRect->right - 6, pRect->bottom-1);
			...
			if (m_bDrawBorder)
			{
				if (m_bIsOnglet == FALSE)
				{
					if (m_bIsFlecheG == FALSE && m_bIsFlecheD == FALSE)
					{
						...
						CPen pen3DDKShadow		(PS_SOLID, 0, GetSysColor(COLOR_3DDKSHADOW));	// Dark shadow
						CPen pen3DDKHighLight	(PS_SOLID, 0, GetSysColor(COLOR_3DHIGHLIGHT));	// High Light
						CPen pen3DShadow		(PS_SOLID, 0, GetSysColor(COLOR_3DSHADOW));		// Shadow
						CPen pen3DLight			(PS_SOLID, 0, GetSysColor(COLOR_3DLIGHT));		// Light

						// on dessine les bords bas et droite en clair
						CPen* pOldPen = pDC->SelectObject(&pen3DDKHighLight);
						pDC->MoveTo(pRect->left +1		, pRect->bottom - 1);
						pDC->LineTo(pRect->right - 1	, pRect->bottom - 1);
						pDC->LineTo(pRect->right - 1	, pRect->top - 1);

						// on dessine le liseré clair
						pDC->SelectObject(&pen3DLight);
						pDC->MoveTo(pRect->left + 2		, pRect->bottom - 3);
						pDC->LineTo(pRect->left + 2		, pRect->top + 2);
				...
				else
				{
					...
					else
					{
						...
						pDC->LineTo(pRect->left + 6, pRect->bottom-1);
						pDC->LineTo(pRect->right - 6, pRect->bottom-1);
						pDC->LineTo(pRect->right, pRect->top - 5);

						// Anti-Aliasing  (A voir !!!!)
						/*
						CPen penBtnShadow(PS_SOLID, 0, GetSysColor(COLOR_BTNSHADOW));   // Dark gray						pDC->LineTo(pRect->left + 6, pRect->bottom-1);
						pOldPen = pDC->SelectObject(&penBtnShadow);
						pDC->MoveTo(pRect->left + 5, pRect->bottom-2);
						pDC->LineTo(pRect->left - 1, pRect->top+1);
						pDC->MoveTo(pRect->right - 5, pRect->bottom-2);
						pDC->LineTo(pRect->right + 1 , pRect->top+1);
						*/
						pDC->SelectObject(pOldPen);
					}
			...
			if (m_bDrawBorder)
			{
				if (m_bIsOnglet == FALSE)
				{
					if (m_bIsFlecheG == FALSE && m_bIsFlecheD == FALSE)
					{
						...
						// on dessine les bords bas et droite en foncé
						CPen* pOldPen = pDC->SelectObject(&pen3DDKShadow);
						pDC->MoveTo(pRect->left +1		, pRect->bottom - 2);
						...
						pDC->SelectObject(&pen3DShadow);
						pDC->MoveTo(pRect->left +2		, pRect->bottom - 3);
						pDC->LineTo(pRect->right - 3	, pRect->bottom - 3);
						pDC->LineTo(pRect->right - 3	, pRect->top - 2);

						// on dessine les bords haut et gauche en clair
						pDC->SelectObject(&pen3DDKHighLight);
						pDC->MoveTo(pRect->left + 1		, pRect->bottom - 2);
						pDC->LineTo(pRect->left + 1		, pRect->top + 1);
						pDC->LineTo(pRect->right - 1	, pRect->top + 1);
...
{
	CBtnSTLabel::OnRButtonDown(nFlags, point);

	// Récupére la fenêtre d'appel
	CWnd*	pWnd = GetParent();

	// Envoi message click bouton droit à la fenêtre
	pWnd->SendMessage(WM_BTNST_LBUTTONDOWN); 
}

/////////////////////////////////////////////////////////////////////////////////////////
// Positionne l'avancement progress bar du bouton
void CButtonST::SetPrctProgress(int Prct)
{
	if (m_bIsModeProgress)
	{
		// Récup la valeur en cours du progress bar
		m_PrctProgress = Prct;	
	}
}

void CButtonST::ResetPrctProgress()
{
	// Init progress bar 
	m_PrctProgress = 0;
}