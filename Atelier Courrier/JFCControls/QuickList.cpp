...
bool CQuickList::IsSelected(const int item, const int subitem)
{
	...
	{
		if ( FullRowSelect() || //Full row select or...
			subitem == 0 )		//First column?	
		{
			// has focus?
			if (m_hWnd != ::GetFocus())
			{
								...
								CDC* pDC)
{
...
#endif
	{
		...
		//Fill background if not progressbar
		FillSolidRect(	pDC,
						rect,
	...
	{		
		...
		//Draw transparent if progress bar
		pDC->SetTextColor( GetTextColor(id, id.m_noSelection) );
		pDC->SetBkColor( GetBackColor(id, id.m_noSelection) );
...
#endif
		{
			if( GetBackColor(id, id.m_noSelection) == TRANSPARENTCOLOR)
				pDC->SetBkMode(TRANSPARENT);
			else
				pDC->SetBkMode(OPAQUE);

			rect.left += 2;
			pDC->DrawText(text, &rect, textFormat); //nFor
		}
#ifndef QUICKLIST_NOPROGRESSBAR
		else
		{
...
CEdit* CQuickList::EditSubItem (int Item, int Column)
{
...

	{
		...
		strcpy_s(dispinfo.item.pszText, dispinfo.item.cchTextMax, text);
...
int CQuickList::OnToolHitTest(CPoint point, TOOLINFO * pTI) const
{
	...
	// Juillet 2013 , bizarre selon la position mouse on peut arriver à nSubItem = -1 ???
	...
	if(HitTest(pt, nItem, nSubItem) )
	{
		...
		if (!strToolTip.IsEmpty())
		{
			...
			else if(pNMHDR->code == TTN_NEEDTEXTW)
			{
				...
				mbstowcs_s(&numConv, pTTTW->szText, 80, strToolTip, 80);
			}
#else
			if (pNMHDR->code == TTN_NEEDTEXTA)
...
/*
///////////////////////////////////////////////////////////////////////////////////////
// Procédure de comparaison sur les items
static int CALLBACK 
MyCompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
   // lParamSort contains a pointer to the list view control.
	CQuickList* pQuickList = (CQuickList*) lParamSort;
	CString    strItem1 = pQuickList->GetItemText(lParam1, 0); // m_SortColumn);
	CString    strItem2 = pQuickList->GetItemText(lParam2, 0); // m_SortColumn);
	
	return CCompareTitre::CompareTitre(strItem1, strItem2);
}

void CQuickList::OnLvnColumnclick(NMHDR *pNMHDR, LRESULT *pResult)
{
