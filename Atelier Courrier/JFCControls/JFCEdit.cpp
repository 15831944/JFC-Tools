// JFCEdit.cpp : implementation file for CEdit-derived classes
...
BEGIN_MESSAGE_MAP(CJFCEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCEdit)
	ON_WM_KEYDOWN()
	//}}AFX_MSG_MAP
...
CString CJFCEdit::GetText() const
{
	CString strText;
...
CString CJFCEdit::GetTrimmedText() const
{
...
void CJFCEdit::SetText(const CString& strText)
{
	SetWindowText(strText);
}

// Sets the background color to the given rgb.
void CJFCEdit::SetBackgroundColor(COLORREF rgb)
{
...
COLORREF CJFCEdit::GetBackgroundColor() const
{
	CJFCEdit* pThis = const_cast<CJFCEdit*>(this);
...
void CJFCEdit::SetTextColor(COLORREF rgb)
{
	m_rgbText = rgb;
...
COLORREF CJFCEdit::GetTextColor() const
{
...
bool CJFCEdit::ShouldEnter(TCHAR c) const
{
	return true;
}

// Cuts the current selection into the clipboard.
LONG CJFCEdit::OnCut(UINT, LONG)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
...
LONG CJFCEdit::OnClear(UINT wParam, LONG lParam)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
...
LONG CJFCEdit::OnPaste(UINT, LONG)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
...
BOOL CJFCEdit::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pLResult) 
{
...
LONG CJFCEdit::OnSetText(UINT wParam, LONG lParam)
{
	LONG nResult = (LONG)CEdit::Default();

	CString strText = GetValidText();
	if (strText != (LPCTSTR)(LONG_PTR)lParam)
		SetWindowText(strText);
...
CJFCEdit::SelectionSaver::SelectionSaver(CEdit* pEdit, int nStart, int nEnd) :
	m_pEdit(pEdit),
	m_nStart(nStart),
	m_nEnd(nEnd)
{
...
CJFCEdit::SelectionSaver::~SelectionSaver()
{
	if (m_pEdit)
...
void CJFCEdit::SelectionSaver::MoveTo(int nStart, int nEnd)
{
	ASSERT(nStart <= nEnd);
...
void CJFCEdit::SelectionSaver::MoveBy(int nStart, int nEnd)
{
	m_nStart += nStart;
	m_nEnd += nEnd;
...
void CJFCEdit::SelectionSaver::MoveBy(int nPos)
{
	m_nStart += nPos;
	m_nEnd += nPos;
}
...
// Changes both the start and end values by nPos.
void CJFCEdit::SelectionSaver::operator+=(int nPos)
{
	MoveBy(nPos);
}

// Returns the value for the selection's start.
int CJFCEdit::SelectionSaver::GetStart() const
{
	return m_nStart;
}

// Returns the value for the selection's end.
int CJFCEdit::SelectionSaver::GetEnd() const
{
	return m_nEnd;
}

// Updates the selection's start and end with the current selection.
void CJFCEdit::SelectionSaver::Update()
{
	if (m_pEdit)
...
void CJFCEdit::SelectionSaver::Disable()
{
	m_pEdit = NULL;
}
...
CJFCEdit::Behavior::Behavior(CJFCEdit* pEdit) :
	m_pEdit(pEdit),
	m_uFlags(0)
{
	ASSERT(m_pEdit);
...
// Destroys the object (virtual).
CJFCEdit::Behavior::~Behavior()
{
}

// Adds and removes flags from the behavior and then redraws the control
void CJFCEdit::Behavior::ModifyFlags(UINT uAdd, UINT uRemove)
{
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::AlphanumericBehavior

// Constructs the object using the given set of strInvalidChars
CJFCEdit::AlphanumericBehavior::AlphanumericBehavior(CJFCEdit* pEdit, int nMaxChars /*= 0*/, const CString& strInvalidChars /*= _T("%'*\"+?><:\\"")*/) :
	Behavior(pEdit),
...
void CJFCEdit::AlphanumericBehavior::SetInvalidCharacters(const CString& strInvalidChars)
{
...
const CString& CJFCEdit::AlphanumericBehavior::GetInvalidCharacters() const
{
	return m_strInvalidChars;
}

// Sets the maximum number of characters to allow for input.
void CJFCEdit::AlphanumericBehavior::SetMaxCharacters(int nMaxChars)
{
...
int CJFCEdit::AlphanumericBehavior::GetMaxCharacters() const
{
	return m_nMaxChars;
}

// Returns the control's value in a valid format.
CString CJFCEdit::AlphanumericBehavior::_GetValidText() const
{
...
void CJFCEdit::AlphanumericBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::MaskedBehavior

// Constructs the object using the given mask.
CJFCEdit::MaskedBehavior::MaskedBehavior(CJFCEdit* pEdit, const CString& strMask /*= _T("")*/) :
	Behavior(pEdit),
	m_strMask(strMask)
{
...
// Returns the mask
const CString& CJFCEdit::MaskedBehavior::GetMask() const
{
	return m_strMask;
}

// Sets the mask and redraws the control to accomodate it
void CJFCEdit::MaskedBehavior::SetMask(const CString& strMask)
{
...
CString CJFCEdit::MaskedBehavior::GetNumericText() const
{
	CString strText = m_pEdit->GetText();
	CString strResult;
...
// Returns the control's value in a valid format.
CString CJFCEdit::MaskedBehavior::_GetValidText() const
{
	CString strText = m_pEdit->GetText();
	...
	for (int iPos = 0, iMaskPos = 0, nLen = strText.GetLength(); iPos < nLen; iPos++, iMaskPos++)
	{
		...
		int iMaskChar = 0;
		int nMaskCharCount = (int)m_arraySymbols.GetSize();
		for (; iMaskChar < nMaskCharCount; iMaskChar++)
		{
			...
			for (; iMaskPos < nMaskLen; iMaskPos++)
			{
				...
				{
					int iMaskChar2 = 0;
					for (; iMaskChar2 < nMaskCharCount; iMaskChar2++)
					{
		...
		if (iMaskChar == nMaskCharCount)
		{
			...
			{
				// Match the character to any of the symbols
				for (iMaskChar = 0, nMaskCharCount = (int)m_arraySymbols.GetSize(); iMaskChar < nMaskCharCount; iMaskChar++)
				{
...
// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCEdit::MaskedBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	// Check to see if it's read only
	if (m_pEdit->IsReadOnly())
		return;
	...
	int nMaskCharCount = (int)m_arraySymbols.GetSize();
	for (int iMaskChar = 0; iMaskChar < nMaskCharCount; iMaskChar++)
	{
		const Symbol& mc = m_arraySymbols[iMaskChar];
		if (cMask == mc)
		{
	...
	int iMaskChar = 0;
	nMaskCharCount = (int)m_arraySymbols.GetSize(); 
	for (;iMaskChar < nMaskCharCount; iMaskChar++)
	{
		const Symbol& mc = m_arraySymbols[iMaskChar];
...
void CJFCEdit::MaskedBehavior::_OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	switch (uChar)
	{
		case VK_DELETE:
...
CJFCEdit::MaskedBehavior::SymbolArray& CJFCEdit::MaskedBehavior::GetSymbolArray()
{
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::MaskedBehavior::Symbol

// Constructs the object -- needed for CArray
CJFCEdit::MaskedBehavior::Symbol::Symbol() :
	m_cSymbol(0),
	m_fnValidation(NULL),
	m_fnConversion(NULL)
{
...
CJFCEdit::MaskedBehavior::Symbol::Symbol(TCHAR cSymbol, ValidationFunction fnValidation, ConversionFunction fnConversion /*= NULL*/) :
	m_cSymbol(cSymbol),
...
bool CJFCEdit::MaskedBehavior::Symbol::Validate(TCHAR c) const
{
	if (m_fnValidation)
...
TCHAR CJFCEdit::MaskedBehavior::Symbol::Convert(TCHAR c) const
{
	if (m_fnConversion)
...
void CJFCEdit::MaskedBehavior::Symbol::Set(TCHAR cSymbol)
{
	m_cSymbol = cSymbol;
}

// Returns the character for the symbol to be used in the mask.
TCHAR CJFCEdit::MaskedBehavior::Symbol::Get() const
{
	return m_cSymbol;
}

// Returns the character for the symbol to be used in the mask.
CJFCEdit::MaskedBehavior::Symbol::operator TCHAR() const
{
	return m_cSymbol;
}
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::NumericBehavior

// Constructs the object using the given nMaxWholeDigits and nMaxDecimalPlaces.
CJFCEdit::NumericBehavior::NumericBehavior(CJFCEdit* pEdit, int nMaxWholeDigits /*= 9*/, int nMaxDecimalPlaces /*= 4*/) :
	Behavior(pEdit),
	...
	m_cGroupSeparator(','),
	m_nDigitsInGroup(0),
	m_dMin(JFC_MIN_NUMBER),	
	m_dMax(JFC_MAX_NUMBER)	
{
...
void CJFCEdit::NumericBehavior::SetMaxWholeDigits(int nMaxWholeDigits)
{
...
int CJFCEdit::NumericBehavior::GetMaxWholeDigits() const
{
	return m_nMaxWholeDigits;
}

// Sets the maximum number of digits after the decimal point.
void CJFCEdit::NumericBehavior::SetMaxDecimalPlaces(int nMaxDecimalPlaces)
{
...
int CJFCEdit::NumericBehavior::GetMaxDecimalPlaces() const
{
	return m_nMaxDecimalPlaces;
}

// Sets whether the negative sign is allowed in the number or not.
void CJFCEdit::NumericBehavior::AllowNegative(bool bAllowNegative /*= true*/)
{
...
bool CJFCEdit::NumericBehavior::IsNegativeAllowed() const
{
	return m_bAllowNegative;
}

// Sets the number of digits to be grouped together (if any).
void CJFCEdit::NumericBehavior::SetDigitsInGroup(int nDigitsInGroup)
{
...
int CJFCEdit::NumericBehavior::GetDigitsInGroup() const
{
	return m_nDigitsInGroup;
}

// Sets the character to use for the decimal point and the group separator (thousands)
void CJFCEdit::NumericBehavior::SetSeparators(TCHAR cDecimal, TCHAR cGroup)
{
	ASSERT(cDecimal);
	ASSERT(cGroup);
...
void CJFCEdit::NumericBehavior::GetSeparators(TCHAR* pcDecimal, TCHAR* pcGroup) const
{
	if (pcDecimal)
...
void CJFCEdit::NumericBehavior::SetPrefix(const CString& strPrefix)
{
...
const CString& CJFCEdit::NumericBehavior::GetPrefix() const
{
	return m_strPrefix;
}

// Parses the given strMask to set the control's configuration.
void CJFCEdit::NumericBehavior::SetMask(const CString& strMask)
{
	int nDecimalPos = -1;
	int nGroupingPos = -1;
...
CString CJFCEdit::NumericBehavior::GetMask() const
{
	CString strMask;
	...
	int iDigit = 0;
	for (; iDigit < m_nMaxDecimalPlaces; iDigit++)
		strMask += '0';
...
void CJFCEdit::NumericBehavior::SetRange(double dMin, double dMax)
{
	ASSERT(dMin <= dMax);
...
void CJFCEdit::NumericBehavior::GetRange(double* pdMin, double* pdMax) const
{
	if (pdMin)
		*pdMin = m_dMin;
	if (pdMax)
...
int CJFCEdit::NumericBehavior::GetGroupSeparatorCount(const CString& strText) const
{
	int nSepCount = 0;
	int nLen = strText.GetLength();
...
CString CJFCEdit::NumericBehavior::GetNumericText(const CString& strText, bool bConvertToNumericSigns /*= false*/) const
{
	CString strNewText;
...
CString CJFCEdit::NumericBehavior::GetDoubleText(double dText, bool bTrimTrailingZeros /*= true*/) const
{
	CString strText;
...
CString CJFCEdit::NumericBehavior::GetSeparatedText(const CString& strText) const
{
...
void CJFCEdit::NumericBehavior::InsertZeros(CString* pStrText, int nPos, int nCount)
{
	ASSERT(pStrText);
...
// Returns the control's value in a valid format.
CString CJFCEdit::NumericBehavior::_GetValidText() const
{
	CString strText = m_pEdit->GetText();
	CString strNewText;
...
// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCEdit::NumericBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	// Check to see if it's read only
	if (m_pEdit->IsReadOnly())
		return;
...
// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCEdit::NumericBehavior::_OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	switch (uChar)
	{
		case VK_DELETE:
...
void CJFCEdit::NumericBehavior::_OnKillFocus(CWnd* pNewWnd) 
{
...
bool CJFCEdit::NumericBehavior::IsValid() const
{
...
bool CJFCEdit::NumericBehavior::CheckIfValid(bool bShowErrorIfNotValid /*= true*/)
{
...
void CJFCEdit::NumericBehavior::AdjustWithinRange()
{
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::DateBehavior

// Constructs the object with the given control.
CJFCEdit::DateBehavior::DateBehavior(CJFCEdit* pEdit) :
	Behavior(pEdit),
	m_dateMin(JFC_MIN_OLEDATETIME),
	m_dateMax(JFC_MAX_OLEDATETIME),
	m_cSep('/')
{
...
// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCEdit::DateBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	// Check to see if it's read only
	if (m_pEdit->IsReadOnly())
		return;
...
// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCEdit::DateBehavior::_OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	switch (uChar)
	{
		case VK_DELETE:
...
void CJFCEdit::DateBehavior::_OnKillFocus(CWnd* pNewWnd) 
{
	Behavior::_OnKillFocus(pNewWnd);
...
CString CJFCEdit::DateBehavior::GetString(int nValue, bool bTwoDigitWithLeadingZero /*= true*/)
{
	CString strValue;
...
int CJFCEdit::DateBehavior::GetMonthStartPosition() const
{
...
int CJFCEdit::DateBehavior::GetMinMonth() const
{
...
int CJFCEdit::DateBehavior::GetMaxDay() const
{
...
int CJFCEdit::DateBehavior::GetMinDay() const
{
	int nYear = GetValidYear();
	int nMonth = GetValidMonth();
...
int CJFCEdit::DateBehavior::GetMaxDayOfMonth(int nMonth, int nYear)
{
...
TCHAR CJFCEdit::DateBehavior::GetMaxMonthDigit(int nPos) const
{
...
TCHAR CJFCEdit::DateBehavior::GetMinMonthDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
...
bool CJFCEdit::DateBehavior::IsValidMonthDigit(TCHAR c, int nPos) const
{
...
bool CJFCEdit::DateBehavior::IsValidMonth(int nMonth) const
{
	int nYear = GetValidYear();
...
TCHAR CJFCEdit::DateBehavior::GetMaxDayDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
...
TCHAR CJFCEdit::DateBehavior::GetMinDayDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
...
bool CJFCEdit::DateBehavior::IsValidDayDigit(TCHAR c, int nPos) const
{
	return (c >= GetMinDayDigit(nPos) && c <= GetMaxDayDigit(nPos));
}

// Returns true if the given day is valid and falls within the range.
bool CJFCEdit::DateBehavior::IsValidDay(int nDay) const
{
...
bool CJFCEdit::DateBehavior::IsValidYear(int nYear) const
{
...
bool CJFCEdit::DateBehavior::AdjustMaxMonthAndDay()
{
	int nMonth = GetMonth();	
...
bool CJFCEdit::DateBehavior::AdjustMaxDay()
{
	int nDay = GetDay();
...
TCHAR CJFCEdit::DateBehavior::GetMaxYearDigit(int nPos) const
{
...
TCHAR CJFCEdit::DateBehavior::GetMinYearDigit(int nPos, bool bValidYear /*= false*/) const
{
	ASSERT(nPos >= 0 && nPos <= 3);
...
bool CJFCEdit::DateBehavior::IsValidYearDigit(TCHAR c, int nPos) const
{
	return (c >= GetMinYearDigit(nPos) && c <= GetMaxYearDigit(nPos));
}

// Returns the month currently shown on the control or 0.
int CJFCEdit::DateBehavior::GetMonth() const
{
	CString strText = m_pEdit->GetText();
...
int CJFCEdit::DateBehavior::GetValidMonth() const
{
	int nMonth = GetMonth();
...
int CJFCEdit::DateBehavior::GetDay() const
{
	CString strText = m_pEdit->GetText();
...
// If it is less than the minimum allowed, the minimum is returned; 
// if it is more than the maximum allowed, the maximum is returned.
int CJFCEdit::DateBehavior::GetValidDay() const
{
	int nDay = GetDay();
...
int CJFCEdit::DateBehavior::GetYear() const
{
	CString strText = m_pEdit->GetText();
...
// If it is less than the minimum allowed, the minimum is returned; 
// if it is more than the maximum allowed, the maximum is returned.
int CJFCEdit::DateBehavior::GetValidYear() const
{
	int nYear = GetYear();
	if (nYear < m_dateMin.GetYear())
	{
...
void CJFCEdit::DateBehavior::SetMonth(int nMonth)
{
	SelectionSaver selection = m_pEdit;	// remember the current selection
...
void CJFCEdit::DateBehavior::SetDay(int nDay)
{
...
void CJFCEdit::DateBehavior::SetYear(int nYear)
{
...
void CJFCEdit::DateBehavior::SetDate(int nYear, int nMonth, int nDay)
{
...
void CJFCEdit::DateBehavior::SetDate(const CTime& date)
{
...
// Returns true if the control's date is valid and falls within the allowed range.
// If bShowErrorIfNotValid is true, an error message box is shown and the control gets the focus.
bool CJFCEdit::DateBehavior::CheckIfValid(bool bShowErrorIfNotValid /*= true*/)
{
	if (!m_pEdit->IsWindowEnabled())
		return true;
...
void CJFCEdit::DateBehavior::ShowErrorMessage() const
{
...
void CJFCEdit::DateBehavior::SetRange(const CTime& dateMin, const CTime& dateMax)
{
	ASSERT(dateMin >= JFC_MIN_CTIME);
	ASSERT(dateMax <= JFC_MAX_CTIME);
...
void CJFCEdit::DateBehavior::SetRange(const COleDateTime& dateMin, const COleDateTime& dateMax)
{
	ASSERT(dateMin >= JFC_MIN_OLEDATETIME);
	ASSERT(dateMax <= JFC_MAX_OLEDATETIME);
	ASSERT(dateMin <= dateMax);
...
void CJFCEdit::DateBehavior::GetRange(CTime* pDateMin, CTime* pDateMax) const
{
	if (pDateMin)
...
void CJFCEdit::DateBehavior::GetRange(COleDateTime* pDateMin, COleDateTime* pDateMax) const
{
	if (pDateMin)
		*pDateMin = m_dateMin;
	if (pDateMax)
...
bool CJFCEdit::DateBehavior::IsValid(const COleDateTime& date, bool bDateOnly /*= true*/) const
{
...
void CJFCEdit::DateBehavior::SetSeparator(TCHAR cSep)
{
	ASSERT(cSep);
...
TCHAR CJFCEdit::DateBehavior::GetSeparator() const
{
	return m_cSep;
}

// Sets whether the day should be shown before the month or after it.
void CJFCEdit::DateBehavior::ShowDayBeforeMonth(bool bDayBeforeMonth /*= true*/)
{
...
bool CJFCEdit::DateBehavior::IsDayShownBeforeMonth() const
{
	return (m_uFlags & DayBeforeMonth) ? true : false;
}

// Returns the control's value in a valid format.
CString CJFCEdit::DateBehavior::_GetValidText() const
{
	CString strText = m_pEdit->GetText();
	...
	if (GetYear() == 0 && GetMonth() == 0 && GetDay() == 0)
		((CJFCEdit::DateBehavior*)this)->SetDateToToday();
...
CString CJFCEdit::DateBehavior::GetFormattedDate(int nYear, int nMonth, int nDay) const
{
	CString strText;	
	if (m_uFlags & DayBeforeMonth)
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::TimeBehavior

#define JFC_DEFAULT_DATE		1899, 12, 30

// Constructs the object with the given control.
CJFCEdit::TimeBehavior::TimeBehavior(CJFCEdit* pEdit) :
	Behavior(pEdit),
	m_timeMin(JFC_DEFAULT_DATE, 0, 0, 0),
	m_timeMax(JFC_DEFAULT_DATE, 23, 59, 59),
	m_cSep(':'),
	m_strAM(JFC_AM_SYMBOL),
	m_strPM(JFC_PM_SYMBOL),
...
// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCEdit::TimeBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	// Check to see if it's read only
	if (m_pEdit->IsReadOnly())
		return;
...
// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCEdit::TimeBehavior::_OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	switch (uChar)
	{
		case VK_DELETE:
...
// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
// It's used here to check if any action needs to be taken based on the control's value.
void CJFCDateEdit::TimeBehavior::_OnKillFocus(CWnd* pNewWnd) 
{
	Behavior::_OnKillFocus(pNewWnd);
...
int CJFCEdit::TimeBehavior::GetHourStartPosition() const
{
	return m_nHourStart;
}

// Returns the zero-based position of the minute inside the control.
int CJFCEdit::TimeBehavior::GetMinuteStartPosition() const
{
	return m_nHourStart + 3;
}

// Returns the zero-based position of the second inside the control.
int CJFCEdit::TimeBehavior::GetSecondStartPosition() const
{
...
int CJFCEdit::TimeBehavior::GetAMPMStartPosition() const
{
...
int CJFCEdit::TimeBehavior::GetMaxHour(bool b24HourFormat /*= false*/) const
{
...
// Note: This value is not based on the allowed range.
int CJFCEdit::TimeBehavior::GetMinHour(bool b24HourFormat /*= false*/) const
{
...
// Note: This value is not based on the allowed range.
int CJFCEdit::TimeBehavior::GetMaxMinute() const
{
	return 59;
}
...
// Note: This value is not based on the allowed range.
int CJFCEdit::TimeBehavior::GetMinMinute() const
{
	return 0;
}
...
// Note: This value is not based on the allowed range.
int CJFCEdit::TimeBehavior::GetMaxSecond() const
{
	return 59;
}
...
// Note: This value is not based on the allowed range.
int CJFCEdit::TimeBehavior::GetMinSecond() const
{
	return 0;
}

// Returns the digit at the given position (0 or 1) for the maximum value of the hour.
TCHAR CJFCEdit::TimeBehavior::GetMaxHourDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
...
TCHAR CJFCEdit::TimeBehavior::GetMinHourDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
...
bool CJFCEdit::TimeBehavior::IsValidHourDigit(TCHAR c, int nPos) const
{
...
bool CJFCEdit::TimeBehavior::IsValidHour(int nHour, bool b24HourFormat /*= false*/) const
{
...
TCHAR CJFCEdit::TimeBehavior::GetMaxMinuteDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
	return (nPos == 0 ? '5' : '9');
}
...
// Returns the digit at the given position (0 or 1) for the maximum value of the minute.
TCHAR CJFCEdit::TimeBehavior::GetMinMinuteDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);	
    return '0';
}
...
bool CJFCEdit::TimeBehavior::IsValidMinuteDigit(TCHAR c, int nPos) const
{
    return (c >= GetMinMinuteDigit(nPos) && c <= GetMaxMinuteDigit(nPos));
}

// Returns true if the given minute is valid.
bool CJFCEdit::TimeBehavior::IsValidMinute(int nMinute) const
{
...
TCHAR CJFCEdit::TimeBehavior::GetMaxSecondDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);
	return (nPos == 0 ? '5' : '9');
}
...
// Returns the digit at the given position (0 or 1) for the maximum value of the second.
TCHAR CJFCEdit::TimeBehavior::GetMinSecondDigit(int nPos) const
{
	ASSERT(nPos >= 0 && nPos <= 1);	
    return '0';
}
...
bool CJFCEdit::TimeBehavior::IsValidSecondDigit(TCHAR c, int nPos) const
{
    return (c >= GetMinSecondDigit(nPos) && c <= GetMaxSecondDigit(nPos));
}

// Returns true if the given second is valid.
bool CJFCEdit::TimeBehavior::IsValidSecond(int nSecond) const
{
...
void CJFCEdit::TimeBehavior::ShowAMPM()
{
...
void CJFCEdit::TimeBehavior::SetAMPM(bool bAM)
{
...
bool CJFCEdit::TimeBehavior::ChangeAMPM(TCHAR c)
{
	if (IsShowing24HourFormat())
		return false;
...
int CJFCEdit::TimeBehavior::GetAMPMPosition(const CString& strText) const
{
...
bool CJFCEdit::TimeBehavior::IsValidAMPM(const CString& strAMPM) const
{
	return (strAMPM == m_strAM || strAMPM == m_strPM);
}

// Returns the hour currently shown on the control or -1.
int CJFCEdit::TimeBehavior::GetHour() const
{
	CString strText = m_pEdit->GetText();
...
int CJFCEdit::TimeBehavior::GetValidHour(bool b24HourFormat /*= false*/) const
{
	int nHour = GetHour();
...
int CJFCEdit::TimeBehavior::GetMinute() const
{
	CString strText = m_pEdit->GetText();
...
int CJFCEdit::TimeBehavior::GetValidMinute() const
{
	int nMinute = GetMinute();
...
int CJFCEdit::TimeBehavior::GetSecond() const
{
	CString strText = m_pEdit->GetText();
...
int CJFCEdit::TimeBehavior::GetValidSecond() const
{
	int nSecond = GetSecond();
...
CString CJFCEdit::TimeBehavior::GetAMPM() const
{
	CString strText = m_pEdit->GetText();
	int nPos = GetAMPMPosition(strText);
	if (nPos > 0)
...
CString CJFCEdit::TimeBehavior::GetValidAMPM() const
{
	CString strAMPM = GetAMPM();
	if (!IsValidAMPM(strAMPM))
		return m_strAM;			
...
void CJFCEdit::TimeBehavior::SetHour(int nHour)
{
...
void CJFCEdit::TimeBehavior::SetMinute(int nMinute)
{
...
void CJFCEdit::TimeBehavior::SetSecond(int nSecond)
{
...
int CJFCEdit::TimeBehavior::ConvertTo24Hour(int nHour, const CString& strAMPM) const
{
...
int CJFCEdit::TimeBehavior::ConvertToAMPMHour(int nHour, CString* pStrAMPM /*= NULL*/) const
{
...
CTime CJFCEdit::TimeBehavior::GetTime() const
{
    if (IsShowing24HourFormat())
...
COleDateTime CJFCEdit::TimeBehavior::GetOleTime() const
{
    if (IsShowing24HourFormat())
		return COleDateTime(JFC_DEFAULT_DATE, GetHour(), GetMinute(), GetValidSecond());
	return COleDateTime(JFC_DEFAULT_DATE, ConvertTo24Hour(GetHour(), GetAMPM()), GetMinute(), GetValidSecond());
}

// Sets the hour, minute, and second on the control to the given values, which must be valid.
void CJFCEdit::TimeBehavior::SetTime(int nHour, int nMinute, int nSecond /*= 0*/)
{
...
void CJFCEdit::TimeBehavior::SetTime(const CTime& time)
{
...
bool CJFCEdit::TimeBehavior::IsValid(bool bCheckRangeAlso) const
{
	...
	COleDateTime date(JFC_DEFAULT_DATE, nHour, nMinute, nSecond);
...
// If bShowErrorIfNotValid is true, an error message box is shown and the control gets the focus.
bool CJFCEdit::TimeBehavior::CheckIfValid(bool bShowErrorIfNotValid /*= true*/)
{
	if (!m_pEdit->IsWindowEnabled())
		return true;
...
void CJFCEdit::TimeBehavior::ShowErrorMessage() const
{
...
void CJFCEdit::TimeBehavior::SetRange(const CTime& dateMin, const CTime& dateMax)
{
	ASSERT(dateMin >= JFC_MIN_CTIME);
	ASSERT(dateMax <= JFC_MAX_CTIME);
...
void CJFCEdit::TimeBehavior::SetRange(const COleDateTime& dateMin, const COleDateTime& dateMax)
{
	ASSERT(dateMin >= JFC_MIN_OLEDATETIME);
	ASSERT(dateMax <= JFC_MAX_OLEDATETIME);
...
void CJFCEdit::TimeBehavior::GetRange(CTime* pDateMin, CTime* pDateMax) const
{
	if (pDateMin)
...
void CJFCEdit::TimeBehavior::GetRange(COleDateTime* pDateMin, COleDateTime* pDateMax) const
{
	if (pDateMin)
...
// Returns true if the given date is valid and falls within the range.
bool CJFCEdit::TimeBehavior::IsValid(const COleDateTime& date, bool bDateOnly /*= true*/) const
{
...
void CJFCEdit::TimeBehavior::SetSeparator(TCHAR cSep)
{
	ASSERT(cSep);
	ASSERT(!_istdigit(cSep));
...
TCHAR CJFCEdit::TimeBehavior::GetSeparator() const
{
	return m_cSep;
}
...
void CJFCEdit::TimeBehavior::Show24HourFormat(bool bShow24HourFormat /*= true*/)
{
...
void CJFCEdit::TimeBehavior::SetAMPMSymbols(const CString& strAM, const CString& strPM)
{
...
void CJFCEdit::TimeBehavior::GetAMPMSymbols(CString* pStrAM, CString* pStrPM) const
{
	ASSERT(pStrAM);
	ASSERT(pStrPM);
	*pStrAM = m_strAM;
...
// Returns the control's value in a valid format.
CString CJFCEdit::TimeBehavior::_GetValidText() const
{
	CString strText = m_pEdit->GetText();
...
CString CJFCEdit::TimeBehavior::GetFormattedTime(int nHour, int nMinute, int nSecond, const CString& strAMPM /*= _T("")*/) const
{
	if (IsShowing24HourFormat())
	{
...
void CJFCEdit::TimeBehavior::AdjustWithinRange()
{
	// Check if it's already within the range
	if (IsValid())
		return;
...
/////////////////////////////////////////////////////////////////////////////
// CJFCEdit::DateTimeBehavior

CJFCEdit::DateTimeBehavior::DateTimeBehavior(CJFCEdit* pEdit) : 
	DateBehavior(pEdit), 
	TimeBehavior(pEdit),
	Behavior(pEdit)
{ 
...
void CJFCEdit::DateTimeBehavior::SetDateTime(int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond /*= 0*/)
{
...
void CJFCEdit::DateTimeBehavior::SetDateTime(const CTime& dt)
{
...
COleDateTime CJFCEdit::DateTimeBehavior::GetOleDateTime() const
{
...
bool CJFCEdit::DateTimeBehavior::IsValid() const
{
	if (m_uFlags & DateOnly)
...
// Sets the range of allowed date values to the given minimum and maximum CTime values.
void CJFCEdit::DateTimeBehavior::SetRange(const CTime& dateMin, const CTime& dateMax)
{
...
// Sets the range of allowed date values to the given minimum and maximum COleDateTime values.
void CJFCEdit::DateTimeBehavior::SetRange(const COleDateTime& dateMin, const COleDateTime& dateMax)
{
	DateBehavior::SetRange(dateMin, dateMax);
	TimeBehavior::SetRange(dateMin, dateMax);
}
...
void CJFCEdit::DateTimeBehavior::GetRange(CTime* pDateMin, CTime* pDateMax) const
{
...
void CJFCEdit::DateTimeBehavior::GetRange(COleDateTime* pDateMin, COleDateTime* pDateMax) const
{
	DateBehavior::GetRange(pDateMin, pDateMax);
	TimeBehavior::GetRange(pDateMin, pDateMax);
}
...
bool CJFCEdit::DateTimeBehavior::IsValid(const COleDateTime& date, bool bDateOnly /*= true*/) const
{
	if (bDateOnly)
...
void CJFCEdit::DateTimeBehavior::SetSeparator(TCHAR cSep, bool bDate)
{
	if (bDate)
...
TCHAR CJFCEdit::DateTimeBehavior::GetSeparator(bool bDate) const
{
	return (bDate ? DateBehavior::GetSeparator() : TimeBehavior::GetSeparator());
}

// Adds and removes flags from the behavior and then redraws the control
void CJFCEdit::DateTimeBehavior::ModifyFlags(UINT uAdd, UINT uRemove)
{
	UINT uFlags = (m_uFlags & ~uRemove) | uAdd;
...
// Returns the control's value in a valid format.
CString CJFCEdit::DateTimeBehavior::_GetValidText() const
{
...
// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCEdit::DateTimeBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	// Check to see if it's read only
	if (m_pEdit->IsReadOnly())
		return;
...
// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCEdit::DateTimeBehavior::_OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	// Check if we're showing the time only
	if (m_uFlags & TimeOnly)
	{
...
// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
// It's used here to check if any action needs to be taken based on the control's value.
void CJFCEdit::DateTimeBehavior::_OnKillFocus(CWnd* pNewWnd)
{
...
void CJFCEdit::DateTimeBehavior::ShowErrorMessage() const
{
...
// Constructs the object using the given set of strInvalidChars
CJFCAlphanumericEdit::CJFCAlphanumericEdit(int nMaxChars /*= 0*/, const CString& strInvalidChars /*= _T("%'*\"+?><:\\"")*/) :
	AlphanumericBehavior(this, nMaxChars, strInvalidChars)
{
}

BEGIN_MESSAGE_MAP(CJFCAlphanumericEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCAlphanumericEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Returns the control's value in a valid format.
CString CJFCAlphanumericEdit::GetValidText() const
{
	return _GetValidText();
}

// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCAlphanumericEdit::OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	_OnChar(uChar, nRepCnt, nFlags);
}
...
CJFCMaskedEdit::CJFCMaskedEdit(const CString& strMask /*= _T("")*/) :
	MaskedBehavior(this, strMask)
{
}

BEGIN_MESSAGE_MAP(CJFCMaskedEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCMaskedEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Returns the control's value in a valid format.
CString CJFCMaskedEdit::GetValidText() const
{
	return _GetValidText();
}

// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCMaskedEdit::OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	_OnChar(uChar, nRepCnt, nFlags);
}

// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCMaskedEdit::OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags)
{
...
// Constructs the object using the given nMaxWholeDigits and nMaxDecimalPlaces.
CJFCNumericEdit::CJFCNumericEdit(int nMaxWholeDigits /*= 9*/, int nMaxDecimalPlaces /*= 4*/) :
	NumericBehavior(this, nMaxWholeDigits, nMaxDecimalPlaces)
{
}

BEGIN_MESSAGE_MAP(CJFCNumericEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCNumericEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Returns the control's value in a valid format.
CString CJFCNumericEdit::GetValidText() const
{
	return _GetValidText();
}

// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCNumericEdit::OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	_OnChar(uChar, nRepCnt, nFlags);
}

// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCNumericEdit::OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	_OnKeyDown(uChar, nRepCnt, nFlags);
}

// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
void CJFCNumericEdit::OnKillFocus(CWnd* pNewWnd) 
{
...
CJFCIntegerEdit::CJFCIntegerEdit(int nMaxWholeDigits /*= 9*/) :
	CJFCNumericEdit(nMaxWholeDigits, 0)
{
}

BEGIN_MESSAGE_MAP(CJFCIntegerEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCIntegerEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Constructs the object using the given numeric strMask.
CJFCCurrencyEdit::CJFCCurrencyEdit()
{
	m_nDigitsInGroup = 3;
...
BEGIN_MESSAGE_MAP(CJFCCurrencyEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCCurrencyEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
CJFCDateEdit::CJFCDateEdit()
	:	CJFCEdit::DateBehavior(this)
	,	CJFCEdit::Behavior(this)
{
}


BEGIN_MESSAGE_MAP(CJFCDateEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCDateEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Returns the control's value in a valid format.
CString CJFCDateEdit::GetValidText() const
{
	return _GetValidText();
}

// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCDateEdit::OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	_OnChar(uChar, nRepCnt, nFlags);
}

// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCDateEdit::OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	_OnKeyDown(uChar, nRepCnt, nFlags);
}

// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
void CJFCDateEdit::OnKillFocus(CWnd* pNewWnd) 
{
	_OnKillFocus(pNewWnd);
}

// Handles the WM_PASTE message to ensure that the text being pasted is a valid date.
LONG CJFCDateEdit::OnPaste(UINT, LONG)
{
	return _OnPaste(0, 0);
}

bool CJFCDateEdit::ShouldEnter(TCHAR c) const
{
	if (c == VK_BACK || (c >='0' && c<='9') )
		return true;
	else
		return false;
}
...
CJFCTimeEdit::CJFCTimeEdit()
	:	CJFCEdit::TimeBehavior(this)
	,	CJFCEdit::Behavior(this)	
{
}


BEGIN_MESSAGE_MAP(CJFCTimeEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCTimeEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Returns the control's value in a valid format.
CString CJFCTimeEdit::GetValidText() const
{
	return _GetValidText();
}

// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCTimeEdit::OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	_OnChar(uChar, nRepCnt, nFlags);
}

// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCTimeEdit::OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	_OnKeyDown(uChar, nRepCnt, nFlags);
}

// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
void CJFCTimeEdit::OnKillFocus(CWnd* pNewWnd) 
{
	_OnKillFocus(pNewWnd);
}

// Handles the WM_PASTE message to ensure that the text being pasted is a valid time.
LONG CJFCTimeEdit::OnPaste(UINT, LONG)
{
	return _OnPaste(0, 0);
}
...
CJFCDateTimeEdit::CJFCDateTimeEdit()
	:	CJFCEdit::DateTimeBehavior(this)
	,	CJFCEdit::Behavior(this)
{
}


BEGIN_MESSAGE_MAP(CJFCDateTimeEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCDateTimeEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Returns the control's value in a valid format.
CString CJFCDateTimeEdit::GetValidText() const
{
	return _GetValidText();
}

// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCDateTimeEdit::OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	_OnChar(uChar, nRepCnt, nFlags);
}

// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCDateTimeEdit::OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags) 
{
	_OnKeyDown(uChar, nRepCnt, nFlags);
}

// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
void CJFCDateTimeEdit::OnKillFocus(CWnd* pNewWnd) 
{
	_OnKillFocus(pNewWnd);
}

// Handles the WM_PASTE message to ensure that the text being pasted is a valid date.
LONG CJFCDateTimeEdit::OnPaste(UINT, LONG)
{
	return _OnPaste(0, 0);
}
...
CJFCMultiMaskedEdit::CJFCMultiMaskedEdit()
	:	CJFCEdit::AlphanumericBehavior(this)
	,	CJFCEdit::NumericBehavior(this)
	,	CJFCEdit::MaskedBehavior(this)
	,	CJFCEdit::DateTimeBehavior(this)
	,	CJFCEdit::Behavior(this)
...
// Returns the mask
const CString& CJFCMultiMaskedEdit::GetMask() const
{
	return MaskedBehavior::m_strMask;
}

// Sets the mask and thereby the edit box's behavior
void CJFCMultiMaskedEdit::SetMask(const CString& strMask)
{
	int nLen = strMask.GetLength();
...
// Returns the control's value in a valid format.
CString CJFCMultiMaskedEdit::GetValidText() const
{
	ASSERT(m_pCurrentBehavior);
	return m_pCurrentBehavior->_GetValidText();
}

BEGIN_MESSAGE_MAP(CJFCMultiMaskedEdit, CEdit)
	//{{AFX_MSG_MAP(CJFCMultiMaskedEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
// Handles the WM_CHAR message, which is called when the user enters a regular character or Backspace
void CJFCMultiMaskedEdit::OnChar(UINT uChar, UINT nRepCnt, UINT c)
{
	ASSERT(m_pCurrentBehavior);
...
// Handles the WM_KEYDOWN message, which is called when the user enters a special character such as Delete or the arrow keys.
void CJFCMultiMaskedEdit::OnKeyDown(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	ASSERT(m_pCurrentBehavior);
...
// Handles the WM_KILLFOCUS message, which is called when the user leaves the control.
void CJFCMultiMaskedEdit::OnKillFocus(CWnd* pNewWnd) 
{
	ASSERT(m_pCurrentBehavior);
...
LONG CJFCMultiMaskedEdit::OnPaste(UINT wParam, LONG lParam)
{
	ASSERT(m_pCurrentBehavior);
