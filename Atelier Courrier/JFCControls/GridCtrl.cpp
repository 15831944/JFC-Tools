...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    // the new system colours.
 	m_crSelColour         = ::GetSysColor(COLOR_HIGHLIGHT);
    ...
    m_bFitToPage		  = FALSE;      // use size-to-width printing
...
void CGridCtrl::SetupDefaultCells()
{
    ...
    m_cellDefault.SetSelClr(m_crSelColour); 
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage, POINT point) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    if(!IsValid(FocusCell))
        SendMessageToParent(-1, -1, NM_RCLICK, point);
    else
    {
        SetFocusCell(-1,-1);
        ...
        SendMessageToParent(FocusCell.row, FocusCell.col, NM_RCLICK, point);
    }
}
#endif
...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    m_CharSize.cx /= 52;
    int nxMargins = (m_nLeftMargin+m_nRightMargin)*m_CharSize.cx;
    int nyMargins = (m_nHeaderHeight+m_nFooterHeight + 2*m_nGap)*m_CharSize.cy;
	...
	{
		m_PaperSize = CSize(pDC->GetDeviceCaps(HORZRES), pDC->GetDeviceCaps(VERTRES));
		if(GetVirtualWidth()>GetVirtualHeight())
		{
			m_LogicalPageSize.cx = GetVirtualWidth()+nxMargins;
	#ifdef _WIN32_WCE
		...
		{//////
			m_LogicalPageSize.cy = GetVirtualHeight()+nyMargins;
	#ifdef _WIN32_WCE
			m_LogicalPageSize.cx = (m_LogicalPageSize.cy * m_PaperSize.cx) / m_PaperSize.cy;
	#else
			m_LogicalPageSize.cx = MulDiv(m_LogicalPageSize.cy, m_PaperSize.cx, m_PaperSize.cy);
	#endif
		}////////

	}
	if(!m_bWysiwygPrinting && !m_bFitToPage)
    {
        m_PaperSize = CSize(pDC->GetDeviceCaps(HORZRES), pDC->GetDeviceCaps(VERTRES));
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    if( !strCenter.IsEmpty() )
		pDC->DrawText( strCenter, &rc, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER);
    if( !strRight.IsEmpty() )
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    strLeft.Format(_T("Page %d"), pInfo->m_nCurPage, pInfo->GetMaxPage() );
