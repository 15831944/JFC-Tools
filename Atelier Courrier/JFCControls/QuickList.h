...
#define WM_QUICKLIST_EDITINGLOSTFOCUS	(WM_USER + 1982)
...
class AFX_EXT_CLASS CQuickList : public CListCtrl
{
// Construction
public:
	CQuickList();
	...
	class AFX_EXT_CLASS CListItemData;
	class AFX_EXT_CLASS CNavigationTest;
	class AFX_EXT_CLASS CListHitInfo;
...
protected:
	...
	void FillSolidRect(CDC* pDC, const RECT rect, const COLORREF color);
	...
	void SortColonne(JUnt32 NoColonne);
...
class AFX_EXT_CLASS CQuickList::CListItemData
{
public:
	CListItemData();
...
#ifndef QUICKLIST_NOTOOLTIP
	//Tool tip text. Note: Don't forget to call EnableToolTips() to enable tool tips. 
	CString m_tooltip;
#endif

	//Set this to true if you don't want to draw a selection mark even if this item is selected.
	//Default value: false
	bool m_noSelection;
	...
	struct CListTextStyle
	{
		...
		//Default value: DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS 
	...
	struct CListImage
	{
		//The image position in the image list. -1 if no image.
		//Default value: -1
		int m_imageID;

		//The image list where the image is.
		//Default value: A pointer to the image list in the list control that is used small images (LVSIL_SMALL)
		CImageList* m_imageList;

		//Set true if you don't want to draw selection mark if the item is selection
		//Default value: true
		bool m_noSelection;
		...
		//Blend if the image is selected. Use ILD_BLEND25 or ILD_BLEND50, or 0 if you don't want to use this feature.
	...
	struct CListButton
	{
		...
		//Use DFCS_BUTTONRADIO for radio button, DFCS_BUTTONPUSH for push button.
		...
		//Set this to true if you don't want to draw selection mark under the control.
		//Default value: true
		bool m_noSelection;
	...
	{
		//Note: The m_text member specifies the text in the progress bar

		//The max value of progress bar. Use -1 to disable progress bar.
		//The min value is supposed to be 0.
		//Default value: -1
		int m_maxvalue;

		//The value the progress bar has. The width of the progress bar is calculated
		//with use m_value and m_maxvalue.
		//Default value: 0
		int m_value;
		...
		//How to draw the edge. Use 0 for no edge. See CDC::DrawEdge for different styles.
	...
	struct CListColors
	{
		...
		//These colors are used to draw selected items in the "navigation column"
		#ifndef QUICKLIST_NONAVIGATION
...
class AFX_EXT_CLASS CQuickList::CNavigationTest
{
public:
...
class AFX_EXT_CLASS CQuickList::CListHitInfo
{
public:
	int m_item;
	int m_subitem;
