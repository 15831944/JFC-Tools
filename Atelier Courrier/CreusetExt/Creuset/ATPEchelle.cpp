...
//////////////////
// le constructeur
IMPLEMENT_DYNCREATE(CATPEchelle, JFCSplitterPane)

CATPEchelle::CATPEchelle()
{
...
JVoid CATPEchelle::SetDocument(JATPDocCreuset* pATPDocCreuset)
{
	...
	// m_T2.SetDocument(pATPDocCreuset);
...
JVoid CATPEchelle::Update()
{
	...
	m_T2.SetPrctProgress(m_pATPDocCreuset->m_PrctApport); 
	m_T2.RedrawWindow(); 
	// m_T2.Update();
...
BEGIN_MESSAGE_MAP(CATPEchelle, JFCSplitterPane)
	//{{AFX_MSG_MAP(CATPEchelle)
	ON_WM_CREATE()
	ON_BN_CLICKED(MSG_CR_TOUS,		OnBtnTous)
	ON_BN_CLICKED(MSG_CR_AUCUN,		OnBtnAucun)
	ON_BN_CLICKED(MSG_CR_APPORT,	OnBtnApport)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
//////////////////////////////////
// CATPEchelle message handlers

int CATPEchelle::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	// on appelle le gestionnaire de base
	if (this->JFCSplitterPane::OnCreate(lpCreateStruct) == -1) return (-1);

	// on calcule la position des composants
	LONG x1_T1 = CATPEchelle_Metriques::CX_Border();				LONG x2_T1 = x1_T1 + CATPEchelle_Metriques::CX_Ax() + CATPEchelle_Metriques::CX_Bx() + 2*CATPEchelle_Metriques::CX_Cx() + CATPEchelle_Metriques::CX_Dx() + (5 * CATPEchelle_Metriques::CX_Interstice()) - CATPEchelle_Metriques::CX_Ax() / 2;
	LONG y1_T1 = CATPEchelle_Metriques::CY_Border();				LONG y2_T1 = y1_T1 + CATPEchelle_Metriques::CY_T1();

	LONG x1_T2 = x2_T1 + CATPEchelle_Metriques::CX_Interstice();	LONG x2_T2 = x1_T2 + CATPEchelle_Metriques::CX_Ax()/2 - 2 * CATPEchelle_Metriques::CX_Interstice();
	LONG y1_T2 = CATPEchelle_Metriques::CY_Border();				LONG y2_T2 = y1_T2 + CATPEchelle_Metriques::CY_T2();
	...
	LONG x1_HS = x1_D2;                                             LONG x2_HS = x2_D2;
...
//	LONG y1_HS = y2_D2 + CATPEchelle_Metriques::CY_Interstice()+ (CATPEchelle_Metriques::CY_x4()-CATPEchelle_Metriques::CY_HS())/2;	LONG y2_HS = y1_HS + CATPEchelle_Metriques::CY_HS();
	...
	// on crée les composants A1 à T2
	if (m_T1.Create(NULL, WS_VISIBLE, CRect(x1_T1, y1_T1, x2_T1, y2_T1), this, 0) == FALSE) return (-1);
	// if (m_T2.Create(NULL, WS_VISIBLE, CRect(x1_T2, y1_T2, x2_T2, y2_T2), this, 0) == FALSE) return (-1);
	...
	// Bouton Apports
	txt.LoadString(IDS_CR_CALCUL_APPORT);
	if (m_T2.Create(txt, WS_CHILD|WS_VISIBLE, CRect(x1_T2, y1_T2, x2_T2, y2_T2), this, MSG_CR_APPORT) == FALSE) return (-1);
	...
	// Bouton "Apports"
	this->SetMode(&m_T2, MGER_MOVE, MGER_NONE);
...
void CATPEchelle::SetColors()
{
	...
	// Le bouton apport de type progress bar
	m_T2.SetColor( CButtonST::BTNST_COLOR_BK_IN, CATPColors::GetColorLight(CATPColors::COLORCREUSET),CATPColors::COLORCREUSET );
	m_T2.SetColor( CButtonST::BTNST_COLOR_BK_OUT, CATPColors::GetColorMedium(CATPColors::COLORCREUSET),CATPColors::COLORCREUSET );
	m_T2.SetColor( CButtonST::BTNST_COLOR_BK_FOCUS, CATPColors::GetColorDark(CATPColors::COLORCREUSET),CATPColors::COLORCREUSET );
	m_T2.SetFontName(CATPColors::GetFontStaticName());
	m_T2.SetFontSize(CATPColors::GetFontStaticSize());
	m_T2.SetFontBold(0);
	m_T2.SetTypeBtnProgress(TRUE, TRUE); 

	Invalidate();
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Activation calcul des apports
void CATPEchelle::OnBtnApport()
{
	// Envoi message recalcul des apports
	m_pATPDocCreuset->m_ApportARecalculer = true;
	m_pATPDocCreuset->UpdateAllViews(UPDATE_CALC_ECHELLE); 
	
	// Les apports ne sont pas à recalculer
	m_pATPDocCreuset->m_ApportARecalculer = false;
}

void CATPEchelle::OnBtnTous()
{
...
JInt32 CATPEchelle::OnGetNbInsertions(JUnt32 IdSupport) const
{
...
JInt32 CATPEchelle::OnGetNbInsertionsPA(JUnt32 IdSupport) const
{
	// on initialise le nombre d'insertions
	JInt32 NbInsertions = 0;
...
JInt32 CATPEchelle::OnGetNbInsertionsEtude(JUnt32 IdSupport) const
{
	// on initialise le nombre d'insertions
	JInt32 NbInsertions = 0;
...
JVoid CATPEchelle::SuppInsertionSupport(JUnt32 IdxSupport)
{
