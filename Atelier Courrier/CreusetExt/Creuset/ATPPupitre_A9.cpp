...
JVoid CATPPupitre_A9::SetDocument(JATPDocCreuset* pATPDocCreuset)
{
...
BEGIN_MESSAGE_MAP(CATPPupitre_A9, JFCButtonList)
	//{{AFX_MSG_MAP(CATPPupitre_A9)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

// la fonction pour récupérer l'état des éléments
void CATPPupitre_A9::OnGetItemParams(LONG i, LONG j, BtnParams & Params)
{
	// on récupère l'état de l'élément
	Params.m_State = 0;

	// Mode sélection simple et led triangle vers le bas (asc vertical)
	...
	Params.m_Design_Led     =   2; 
	Params.m_Text			=   "";  
}

void CATPPupitre_A9::OnButtonSel(LONG i, LONG j)
{
	// Décalage format affichage vers le bas
	m_pATPDocCreuset->m_DecalAffichFormat ++;
	if (m_pATPDocCreuset->m_DecalAffichFormat >= m_pATPDocCreuset->m_MapIdFormatsSel.GetCount())
		m_pATPDocCreuset->m_DecalAffichFormat = m_pATPDocCreuset->m_MapIdFormatsSel.GetCount()-1;

	// Mise à jour vue pupitre formats
	m_pATPDocCreuset->UpdateAllViews(UPDATE_AFF_CREUSET);
}

void CATPPupitre_A9::OnLedSel(LONG i, LONG j)
{
	// Décalage format affichage vers le bas
	m_pATPDocCreuset->m_DecalAffichFormat ++;
	if (m_pATPDocCreuset->m_DecalAffichFormat >= m_pATPDocCreuset->m_MapIdFormatsSel.GetCount())
...
bool CATPPupitre_A9::OnBeginDrawing(CDC & dc)
{
...
/////////////////////////////////////
// CATPPupitre_A9 message handlers

void CATPPupitre_A9::PreSubclassWindow() 
{
