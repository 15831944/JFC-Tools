...
JVoid CATPGraphGRP::SetDocument(JATPDocCreuset* pATPDocCreuset)
{
...
JFControl & CATPGraphGRP::SetHorzScrollBar(JFCScroll & Scroll)
{
...
JFControl & CATPGraphGRP::GetAbcsisseControl()
{
	// on renvoie le contrôle
	return (m_A2);
}
...
BEGIN_MESSAGE_MAP(CATPGraphGRP, JFCFlatPane)
	//{{AFX_MSG_MAP(CATPGraphGRP)
	ON_WM_CREATE()
//	ON_BN_CLICKED(12, OnBtnPrint)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CATPGraphGRP::OnBtnPrint()
{
...
JVoid CATPGraphGRP::OnUpdate()
{
	...
	if (m_pATPDocCreuset != 0)
	{
		if(m_pATPDocCreuset->m_AFFCreuset.GetEchelleResult())
			...
			this->ShowWindow((m_pATPDocCreuset->m_AFFResultPlan.GetGraphGRPJour()) ? SW_SHOW : SW_HIDE);
...
/////////////////////////////////
// CATPGraphGRP message handlers

int CATPGraphGRP::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
			...
			if (m_pATPDocCreuset->m_ITRHypotheses.IsValid())
			{
				...
				if (Effectif > 0.0)
				{
					...
					for (JInt32 IdxJour = 0; IdxJour < NbJours; IdxJour += 1)
					{
						...
						JFlt64 NbContacts = DATHypothese.m_NbContactsJour.Item(IdxJour);

						// on calcule le GRP du jour
						JFlt64 GRP = (NbContacts * 100.0) / Effectif;
