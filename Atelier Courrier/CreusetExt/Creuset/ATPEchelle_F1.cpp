...
JVoid CATPEchelle_F1::SetDocument(JATPDocCreuset* pATPDocCreuset)
{
...
BEGIN_MESSAGE_MAP(CATPEchelle_F1, JFCGrid)
	//{{AFX_MSG_MAP(CATPEchelle_F1)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
JVoid CATPEchelle_F1::OnDrawCell(CDC & dc, RECT & rect, LONG i, LONG j, CBrush & BrushA, CBrush & BrushB, CBrush & BrushC, CBrush & BrushD)
{
	...
	// on écrit le titre
	CString str;
	str.FormatMessage(IDS_CR_LDPLNM);
	...
	if (m_pATPDocCreuset != 0)
	{
		...
		if (m_pATPDocCreuset->IsValid())
		{
			...
			if (m_pATPDocCreuset->m_AFFEchelle.IsSortByLDPUp())
			{
			...
			else if (m_pATPDocCreuset->m_AFFEchelle.IsSortByLDPDn())
			{
...
JVoid CATPEchelle_F1::OnLButtonDown(LONG i, LONG j)
{
	// on vérifie la validité du document
	if (m_pATPDocCreuset != 0)
	{
...
JVoid CATPEchelle_F1::OnRButtonDown(LONG i, LONG j)
{
	// on ne fait rien
}
...
/////////////////////////////////////
// CATPEchelle_F1 message handlers

void CATPEchelle_F1::PreSubclassWindow() 
{
