...
JVoid CATPGraphCouv::SetDocument(JATPDocCreuset* pATPDocCreuset)
{
...
JFControl & CATPGraphCouv::SetHorzScrollBar(JFCScroll & Scroll)
{
...
JFControl & CATPGraphCouv::GetAbcsisseControl()
{
	// on renvoie le contrôle
	return (m_A2);
}
...
BEGIN_MESSAGE_MAP(CATPGraphCouv, JFCFlatPane)
	//{{AFX_MSG_MAP(CATPGraphCouv)
	ON_WM_CREATE()
//	ON_BN_CLICKED(12, OnBtnPrint)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CATPGraphCouv::OnBtnPrint()
{
/*
	m_btnPrint.ShowWindow(SW_HIDE);
	CString str;
	str = "Couverture cumulée " + GetLabel();
...
JVoid CATPGraphCouv::OnUpdate()
{
	...
	if (m_pATPDocCreuset != 0)
	{
		if(m_pATPDocCreuset->m_AFFCreuset.GetEchelleResult())
			...
			this->ShowWindow((m_pATPDocCreuset->m_AFFResultPlan.GetGraphCouvCumul()) ? SW_SHOW : SW_HIDE);
...
/////////////////////////////////
// CATPGraphCouv message handlers

int CATPGraphCouv::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
CString CATPGraphCouv::GetLabel()
{
	CString Text = "";
	// on vérifie la validité du document
