// CreusetExt.cpp : définit les fonctions d'initialisation pour la DLL.
//
...
static AFX_EXTENSION_MODULE CreusetExtDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de CreusetExt.DLL !\n");
		
		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(CreusetExtDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(CreusetExtDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Arrêt de CreusetExt.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(CreusetExtDLL);
	}
	return 1;   // ok
}
