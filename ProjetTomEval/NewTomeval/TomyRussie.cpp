...
CFicProba* CTomyRussie::LoadPanel(LPCSTR Panel, LPCSTR Etude,int NbIndiv)
{
	TRACE("LoadPanel : Chargement du panel\n");

	CFicProba * FicProba = m_PanelLoader.LoadPanel(m_Param.m_CheminData, Panel, m_Param.m_TypeSupport,NbIndiv);
...
void CTomyRussie::InitialisePanel(LPCSTR panel, LPCSTR etude,int NbIndiv)
{
	...
	m_CodePanel = panel;
	m_Etude     = etude;
	m_FicProba  = this->LoadPanel(m_CodePanel, m_Etude,NbIndiv);
...
#ifdef OPTION_MAROC
				int IdxQuartHeure = ((((Spot.m_QHeure / 100) * 2) + ((Spot.m_QHeure % 100) / 30)) - 6) * 2;		// HeureDebut : 3h00 (300) de la journee
#else
				int IdxQuartHeure = ((((Spot.m_QHeure / 100) * 2) + ((Spot.m_QHeure % 100) / 30)) - 10) * 2;	// HeureDebut : 5h00 (500) de la journee
#endif
