...
void CObj_Printer::CalculNbPage(CDC * pDC, CPrintInfo* pInfo)
{
	...
	short i,idx, MemoLigCorps=0,MemoLigDebut,place,memoidx,ligdebut;
...
void CObj_Printer::DessineBitmap(CDC* pDC, short Nr, CRect* pR, short PDebut, short PFin)
{
	...
	m_Bitmaps[Nr].GetSize( &largeur, &hauteur );
	...
	if ( ( caps & RC_BITBLT ) != 0 )
	{
		// on initialise les anciens objects
		oldbmp = NULL;
		oldpal = NULL;
		...
		// on cree une bitmap compatible avec le dc d'affichage
		newbmp = m_Bitmaps[Nr].CreateBitmap( hdc );

		// on teste si la creation s'est bien passee
		if ( newbmp != NULL )
		{
			// on cree un DC memoire
			hdcmem = ::CreateCompatibleDC( hdc );

			// on teste si la creation s'est bien passee
			if ( hdcmem != NULL )
			{
				// on selectionne la bitmap dans le dc memoire
				oldbmp = (HBITMAP)::SelectObject( hdcmem, newbmp );

				// on teste si le peripherique possede une palette
				if ( ( caps & RC_PALETTE ) != 0 )
				{
					// on cree une palette pour afficher la bitmap
					newpal = m_Bitmaps[Nr].CreatePalette();

					// on teste si la creation s'est bien passe
					if ( newpal != NULL )
					{
						// on selectionne la palette dans le dc
						oldpal = (HPALETTE)::SelectPalette( hdc, newpal, FALSE );

						// on realise la palette
						::RealizePalette ( hdc );
					}
				}
				...
				CalcBitmapRect( &R, largeur, hauteur, resoX, resoY );

				// on effectue la copie
				::StretchBlt( hdc, R.left, R.top, R.Width(), R.Height(), hdcmem, 0, 0, largeur, hauteur, SRCCOPY );

				// on teste si on doit reselectionner l'ancienne palette
				if ( oldpal != NULL )
				{
					// on selectionne la palette dans le dc
					...
					// on detruit la palette
					::DeleteObject ( newpal );
				}

				// on reselectionne l'ancienne bitmap
				::SelectObject( hdcmem, oldbmp );

				// on detruit la bitmap
				::DeleteObject( newbmp );

				// on detruit le dc memoire
				::DeleteDC ( hdcmem );
			}
		}
	}
