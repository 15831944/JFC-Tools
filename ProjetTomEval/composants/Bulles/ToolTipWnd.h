...
public:
	...
	void AddTool(CWnd *pWnd, CString strText,UINT Size=0, COLORREF clrTextColor = 0, int timerDelay = -1, int timerDelayShow = -1);
	...
	void SetDelayShow( int iDelayShow );

	void Show( HWND hWnd, const char *overrideText = NULL, int timerDelay = -1, int timerDelayShow=-1,UINT Size=0);
	void Hide();
...
private:
	...
	void Show( int x, int y, int timerDelayHide, int timerDelayShow , UINT Size);
...
inline void CToolTipWnd::SetDelayShow(int iDelayShow)		{m_iDelayShow = iDelayShow;}
