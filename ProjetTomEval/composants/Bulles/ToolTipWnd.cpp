...
void CToolTipWnd::OnPaint() 
{
   ...
   dc.FrameRgn( &rgnComb, &pBrush, 2, 2 );
   ...
   else
   {
	...
	CRect vrect=m_rectText;
	vrect.bottom=vrect.top;
	dc.DrawText( m_strText, vrect,DT_CALCRECT | DT_CENTER | DT_WORDBREAK );
	int h=(m_rectText.Height()-vrect.Height())/2;
	int l=(m_rectText.Width()-vrect.Width())/2;
	vrect.OffsetRect(l,h);
	dc.DrawText( m_strText, vrect, DT_CENTER | DT_WORDBREAK );
   }
...
//	         TRACE0("Move\n");
         ...
         if ( m_pCurrwnd == NULL ) // was not in a control
         {
            if ( found ) // enters a control (now in a control)
            {
               ...
               Show( pt.x, pt.y, stToolInfo->iTimerDelay, stToolInfo->iTimerDelayShow ,stToolInfo->iSize);
               m_pCurrwnd = wndPt;
            }
...
void CToolTipWnd::AddTool(CWnd *pWnd, CString strText,UINT Size /*=0*/, COLORREF clrTextColor/*=NULL*/, int timerDelay /*= -1*/, int timerDelayShow /*=-1*/)
{
...
void CToolTipWnd::Show( HWND hWnd, const char *overrideText /*= NULL*/, int timerDelay /*= -1*/, int timerDelayShow/*=-1*/,UINT Size /*=0*/)
{
   ...
   if ( timerDelay <= 0 ) // don't permit a tooltip to stay forever
      timerDelay = m_iDelay;
   Show( pt.x, pt.y, timerDelay, timerDelayShow,Size);
...
// --------------------------------------------------------------------------
void CToolTipWnd::Show( int x, int y, int timerDelayToHide, int timerDelayToShow , UINT Size) // force apparition of the tooltip
{
   ...
   // Recalcul de la région
   {
	   ...
	   m_rgn.DeleteObject();
	   m_rgnTri.DeleteObject();
	   m_rgnComb.DeleteObject();
...
void CToolTipWnd::SetSize(int cx, int cy)
{
	m_iWidth=cx;
	m_iHeight=cy;
}
