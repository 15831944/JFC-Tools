...
short CFicTbl::OpenFile(LPCSTR aNomFic,char aMode)
{
	...
	switch(aMode){
		case 'r': 
			r=(m_FicTbl.Open(aNomFic,CFile::modeRead | CFile::typeText, &e)!=0);
		break;

		case 'w':
			r=m_FicTbl.Open(aNomFic,CFile::modeCreate | CFile::modeWrite | CFile::typeText, &e)!=0;
		break;
	}

	if(!r){
		MsgErr.Format("Erreur à l'ouverture du fichier %s (%d).",aNomFic, e.m_cause);
...
{	
	...
	SupBlancsFin(&m_BuffLine[m_Pos[0]]);	//1er champ
...
{	
	...
	m_Pos=(short*)malloc(sizeof(short)*m_NbField);
	m_Pos[0]=x=-1;
