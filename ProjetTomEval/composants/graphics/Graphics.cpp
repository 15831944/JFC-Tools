...
/*	Tables des couleurs d'origines
	...
	*/
	// MODIF REMI: changement des couleurs
	crTable[1]  = RGB(  58,   110,   165);     // bleue foncé SYMPA
	crTable[2]  = RGB(225,   0,   0);     // Red
	crTable[3] = RGB(  0, 128,   0);     // 
	crTable[4]  = RGB(  221,   231, 0);     // moutarde
	crTable[5]  = RGB(255,   0, 255);     // Magenta
	crTable[6]  = RGB(  0, 255, 255);     // Cyan
	crTable[7]  = RGB(128, 128, 128);     // Grey
	crTable[8]  = RGB(128, 128,   0);     // Brown
	...
	crTable[10] = RGB(  0, 128, 128);     // Aqua
	crTable[11] = RGB(128,   0,   0);     // 
	crTable[12]  = RGB(  0, 221,   0);     // Green
	crTable[13] = RGB(  0,   0, 221);     // 
	crTable[14] = RGB(192, 192, 192);     // 
	crTable[15] = RGB(255, 255,   0);     // Yellow
...
void CGraphics::RecalcRects(RECT& rt)
{
	...
	m_ClientRect = rt;
	SetPixelRect(rt);
	// Modif Rémi 9/09/2001
	// On prévoit un tiers de la fenêtre pour dessiner les libellés si
	// l'écriture est verticale sous le graphique
	// Sinon le standard c'est (c'était) un sixième.
	m_bM = (m_Rect.bottom - m_Rect.top) / (m_bOrientationText?3:6);
	m_tM = (m_Rect.bottom - m_Rect.top) / 6/*10*/;
...
void CGraphics::Title(const char* Title, int Pos,int type)
{
// ADD FRED: 28/03/2002 Modification du titre du graphique
// On sépare le titre en deux chaines: la grandeur et (cible,jour,trh/qdh,vague).
#ifdef VERSION_BELGIQUE
	CString grandeur,libelle;
	int pos;
	grandeur = Title;
	pos = grandeur.Find("\n");
	libelle = grandeur.Right(grandeur.GetLength()-pos-1);
	grandeur = grandeur.Left(pos);

	m_LogFont.lfHeight = (int)(m_Size.cx / -10);
	if (m_LogFont.lfHeight > -10) 
		m_LogFont.lfHeight = -10;
#else	m_LogFont.lfHeight = (int)(m_Size.cx / -15.0);
	if (m_LogFont.lfHeight > -15) 
...
#ifdef VERSION_BELGIQUE
			if(type==0)PrintString((GL + GR) / 2,(m_Rect.top + GT)/4, 0, grandeur);
			else PrintString((GL + GR) / 2,((m_Rect.top + GT)/2)-200, 0, grandeur); 
			m_LogFont.lfHeight = m_LogFont.lfHeight+5;
			m_Font = ::CreateFontIndirect(&m_LogFont);
			::SelectObject(m_hDC, m_Font);
			if(type==0)PrintString((GL + GR) / 2,((m_Rect.top + GT)/2)+5, 0, libelle);
			else PrintString((GL + GR) / 2,/*m_Rect.top + GT+5*/((m_Rect.top + GT)/2)+50, 0, libelle);
#else
...
void CGraphics::PrintString(int x, int y, int theta, const char* fmt)
{
	...
	{
		// Bricolage de rémi effectué dans l'urgence pour que lorsque le 
		// texte verticale ne soit pas perturbé par des centrages
		if(!m_bOrientationText)
		{
...
void CGraphics::Grid()
{
	...
	// Modif rémi pour que les espaces entre les pointillés
	// ne soient pas blancs mais de la couleur du fond
