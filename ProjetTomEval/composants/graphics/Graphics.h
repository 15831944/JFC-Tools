...
public:
	...
	// pour permettre le libellé sous le graphique en verticale
	void EnableOrientationTexte(bool bFlag = false)	{ m_bOrientationText = bFlag; }
	
	void DrawBoundary(COLORREF cr, int size);
	void DrawShadow(int n);
	void Title(const char* Title, int Pos = TOP,int type=0);
