...
{                          
	...
	CString txt;
    for (int i = m_nDataRows * m_nDataCols /*- 1 modif rémi du 22/08/01*/; i--;)
	{
		if (*pnData > nMax)
		{
			nMax = *pnData;
...
{                       
	...
	// Modif rémi pour éviter le plantage
	if(nHDiv>0)sizePixelsPerTick.cx = sizeGraph.cx / nHDiv;
	else sizePixelsPerTick.cx = 0;
	if(nVDiv>0)sizePixelsPerTick.cy = sizeGraph.cy / nVDiv;
	else sizePixelsPerTick.cy = 0;
...
{                 
	...
	m_LogFont.lfOrientation= (m_bOrientationText&&!bVert)?900:0;
	m_LogFont.lfEscapement = (m_bOrientationText&&!bVert)?900:0;
	...
	if (m_Font)
	{
		...
		for (int i = 0; i < nNumTicks; i++)
		{
			...
			if (bVert)
				...
				PrintString(nPos, GB + (m_bOrientationText?3*m_bM/5:(m_bM / 5)), 0, buf);
	...
	if (m_Font)
	{
		...
		for (int i = 0; i < nNumTicks; i++)
		{
			if (bVert)
				...
				PrintString(nPos, GB + (m_bOrientationText?4*m_bM/5:m_bM/ 5), 0, m_pColsLable[i].c_str());
			nPos   += nStep;
		}
		::SelectObject(m_hDC, hOldFont);
		::DeleteObject(m_Font);
