...
CCalculProduitFilm::CResultatNbEntree *  CCalculProduitFilm::CalculNbEntre()
{
  ...
  /*
			C'était pour calculer la fréquentation de la sous cible
			FSC=Fc/FEns

  La fonction ci-dessous permet de tenir compte, dès le calcul du nombre de tickets,
  de la sur fréquentation, ce qui revient à calculer le nombre d'entrées ciblées
			...
			for(int idxsem=0;idxsem<filmprod.m_NbSemaine;idxsem++)
			{
				...
				if(idxsem==0)
				{
					...
					for(int idxmod=0;idxmod<theApp.m_Cible.m_NbClasse[filmprod.m_CodeCritCoeff];idxmod++)
					{
						...
						// On s'assure que le nombre de tickets ne dépasse pas la population de la sous cible
...
// Calcul de la population de la sous cible active sur un réseau géographique donné
// Seul les individus compris dans une modalité du critère courant sont pris en compte
...
// Calcul de la population dsegment actif sur un réseau géographique donné
// Seul les individus compris dans une modalité du critère courant sont pris en compte
...
// Prépare la liste des bacs actifs par semaine campagne (par film)
...
// Prépare la liste des bacs actifs pour toute la campagne
...
// retourne la population de la cible en ne considérant seulement les bacs régionaux ou nationaux
