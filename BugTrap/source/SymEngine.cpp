...
BOOL CSymEngine::WriteReportFiles(PCTSTR pszFolderName, CEnumProcess* pEnumProcess)
{
	...
	g_applifiles->WriteFilesInDirectory(pszFolderName);
	/*g_applicationFiles.push_back("bidon.txt");
	
	if (g_applicationFiles.size()>0)
	{
		...
		for(it = g_applicationFiles.begin();it != g_applicationFiles.end();++it)
		{
		//	PathCombine(szFileName, pszFolderName, it->c_str());
			in.open(*it);
			out.open(szFileName);
	...
	//HANDLE hFile = CreateFile("bidon.txt", GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	
//	yop.open("infoPb.txt");

/*	DWORD dwFileSize = GetFileSize(hFile, NULL);
//	yop<<"size: "<<dwFileSize<<std::endl;
	//DWORD dwBufferSize = min(dwFileSize, g_dwMaxBufferSize);
	PBYTE pFileBuffer = new BYTE[dwFileSize];

	
	DWORD dwProcessedNumber = 0;
	BOOL bResult = ReadFile(hFile, pFileBuffer, dwFileSize, &dwProcessedNumber, NULL);
	
	

	HANDLE hFile2 =  CreateFile(szFileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);


	; 

	 // read original file into target

	//WriteFile(hFile2, pFileBuffer, sizeof(pFileBuffer), &dwProcessedNumber, NULL);
//	CloseHandle(hFile2);
	//CloseHandle(hFile);
...
BOOL CSymEngine::ArchiveReportFiles(PCTSTR pszReportFolder, PCTSTR pszArchiveFileName)
{
	...
	//g_arrLogLinks.AddItem(new CLogLink("bidon2.txt"));
