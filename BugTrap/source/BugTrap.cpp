...
extern "C" BUGTRAP_API void APIENTRY BTKantar_SetApplicationFile(TCHAR* fileName)
{
	if (g_applifiles == NULL)
		g_applifiles= new ApplicationFilesManager();
	
	g_applifiles->AddFile(fileName);
	/*CLogFile* pLogFile = EnterLogFunction(iHandle);
	if (! pLogFile)
		return FALSE;
	BOOL bResult = pLogFile->SetLogFlags(dwLogFlags);
...
extern "C" BUGTRAP_API void APIENTRY BTKantar_SetApplicationFilesContainingInfo(TCHAR* _path, TCHAR* _infoInFileName)
{
	if (g_applifiles == NULL)
		g_applifiles= new ApplicationFilesManager();

	g_applifiles->AddFilesContainingStr(_path,_infoInFileName);
}
